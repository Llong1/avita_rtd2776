...
void HotKeyMainMenu(void);            
void ResumeOSDProcess(void);
...
void ResumeOSDProcess(void)
{
   ...
   if(SysRegionCheckAllDisplayState(_DISPLAY_STATE_NO_SIGNAL) == _TRUE)
    {
      if(GET_FACTORY_BURNIN())// alant add
      {
             RTDFactoryBurnInPattern();
      }
      else
       {
    ...
    switch(SysModeGetModeState())
    {
        case _MODE_STATUS_POWER_OFF:
            ...
            if(SysModeGetModeStateChange() == _TRUE)
            {
				...
				PCB_SIGNAL_STATUS(_TRUE);// alant add
            }

            break;

        case _MODE_STATUS_INITIAL: // AC on
        ...
        case _MODE_STATUS_DISPLAY_CHECK:
            ...
            if(SysRegionCheckUserDisplayStateChange() == _TRUE)
            {
                ...
                if(GET_OSD_DISPLAY_MODE() == _OSD_DM_1P)
                {
                    ...
                    {
                        case _DISPLAY_STATE_NO_SIGNAL:// no signal
					...
					//	PCB_SIGNAL_STATUS(_TRUE);// alant add
                       ...
                       if(GET_FACTORY_BURNIN() == _ON)
		               {
		                   ...
		                   return ;
                            ...
                            ScalerTimerActiveTimerEvent(SEC(5), _USER_TIMER_EVENT_SIGNAL_STATUS); ///Wesley_For 32_Status_20171220
                       break;

                        case _DISPLAY_STATE_OOR:// out of range
							PCB_SIGNAL_STATUS(_FALSE);// alant add
							ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_SIGNAL_STATUS);///Wesley_For 32_Status_20171220
				...
				if(GET_OSD_IN_FACTORY_MENU_STATUS() == _FALSE)
				  {
				  if(GET_FACTORY_BURNIN()){
	                             SET_FACTORY_BURNIN(_OFF);// alant debug
	                             RTDNVRamSaveOsdUserData();// call save function
	                             ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_BURN_IN);//alant debug
				    }
        ...
        case _MODE_STATUS_POWER_SAVING:
            ...
            if(SysPowerGetPowerStatus() == _POWER_STATUS_NORMAL)
            {
                ...
                {
                    ///Wesley Modify For DDCCI Hotkey in Sleep Mode///
        ...
        default:
            ...
            if(GET_KEYMESSAGE() < _KEY_AMOUNT)
            {
                ...
                else
                {
                    ...
                    {
                      if(GET_OSD_TIME_OUT() < _OSD_TIMEOUT_MIN)//  if(GET_OSD_TIME_OUT() <= _OSD_TIMEOUT_MIN)
                        {
                            ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_OSD_DISABLE);
                        }
                        else
...
void RTDOsdEventMsgProc(void)
{
    ...
    {
#if 0
        case _OSDEVENT_SHOW_BURNIN_MSG:
            OsdDispOsdMessage(_OSD_DISP_BURNIN_MSG);
            UserCommonInterfacePanelPowerAction(_BACKLIGHT_ON);
	   ...
	   case _OSDEVENT_SHOW_NOCABLE_MSG:
          case _OSDEVENT_SHOW_NOSIGNAL_MSG:
              ...
              UserCommonInterfacePanelPowerAction(_BACKLIGHT_ON);
              // go to sleep mode
              #if(_MODEL_TYPE == _KUI32171)   ///Wesley180309_NoSignal MSG Timming For KUI32(3-Sec)
       ...
       case _OSDEVENT_SHOW_MAINMENU_MSG:
       ...
       break;
        ...
        case _OSDEVENT_SHOW_NOCABLE_MSG:
            ...
            ScalerTimerActiveTimerEvent(SEC(105), _USER_TIMER_EVENT_GO_TO_POWER_SAVING);
             //    #endif
            break;
#endif
...
void MenuNone(void)
{
	...
	case _5_KEY_MESSAGE: // alant : input hot key
    //   case _RIGHT_KEY_MESSAGE:
            ...
            else if(SysModeGetModeState() == _MODE_STATUS_DISPLAY_CHECK || SysModeGetModeState() == _MODE_STATUS_DISPLAY_SETTING)// alant debug
            {
                OsdFuncShowSourceMenuCheck();

                OsdDispHotKeySourceMenu();// resume input source menu
            }
            break;
...
void MenuDisplayFunction(void)
{
    ...
    {
        case _MENU_KEY_MESSAGE:
            if(GET_OSD_DISPLAY_MODE() == _OSD_DM_1P)// go here
            {
        ...
        case _LEFT_KEY_MESSAGE:
            ...
            OsdDispShowInformation();// alant add
...
void MenuAudio(void)
{
    ...
    {
        case _MENU_KEY_MESSAGE:
         SET_OSD_STATE(_MENU_AUDIO_AUDIO_SOURCE);//   SET_OSD_STATE(_MENU_AUDIO_VOLUME);
...
void MenuColorFormat(void)
{
    ...
    switch(GET_KEYMESSAGE())
    {
        ...
        case _EXIT_KEY_MESSAGE:
            ...
            OsdDispClearSelectColor(_DOWN);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_0);
            OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_COLOR_1);
            break;
...
void MenuAdvanceAspect(void)
{
    switch(GET_KEYMESSAGE())
    {
        case _MENU_KEY_MESSAGE:
            ...
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _INSUBSET);
            OsdDispMainMenuOptionSetting(_OPTION_ASPECT_RATIO_TYPE, _SELECT, g_usBackupValue);//GET_OSD_ASPECT_RATIO_TYPE(UserAdjustGetSelectRegionPort()));

            break;

        case _RIGHT_KEY_MESSAGE:
#if 0//#if(_OVERSCAN_SUPPORT == _ON)
...
void MenuAudioVolume(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _EXIT_KEY_MESSAGE:
            ...
            SET_OSD_STATE(_MENU_AUDIO);
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
          OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_1);//   OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_2);
            OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_AUDIO_0);
            break;
...
void MenuOtherTransparency(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _RIGHT_KEY_MESSAGE:
            ...
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
           OsdDispMainMenuOptionSetting(_OPTION_ROTATE_ADJUST, _UNSELECT, GET_OSD_STATUS());//  OsdDispMainMenuOptionSetting(_OPTION_ROTATE_ADJUST, _UNSELECT, GET_OSD_ROTATE_STATUS());
...
void MenuOtherRotate(void)
{
    ...
    {
        case _MENU_KEY_MESSAGE:
            g_usBackupValue =GET_OSD_STATUS();// GET_OSD_ROTATE_STATUS();
            g_usAdjustValue = GET_OSD_STATUS();//GET_OSD_ROTATE_STATUS();

            SET_OSD_STATE(_MENU_OTHER_ROTATE_ADJUST);
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _INSUBSET);
           OsdDispMainMenuOptionSetting(_OPTION_ROTATE_ADJUST, _SELECT,  GET_OSD_STATUS());//   OsdDispMainMenuOptionSetting(_OPTION_ROTATE_ADJUST, _SELECT, GET_OSD_ROTATE_STATUS());
            break;
...
void MenuOtherBorderWidth(void)// user for usb power
{
    switch(GET_KEYMESSAGE())
    {
        case _MENU_KEY_MESSAGE:
#if 1    // alant test
            g_usBackupValue =GET_USB_POWER_STATUS();
            SET_OSD_STATE(_MENU_OTHER_BORDER_WIDTH_ADJUST);
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _INSUBSET);
            OsdDispSliderAndNumber(_MENU_OTHER_BORDER_WIDTH_ADJUST, GET_USB_POWER_STATUS());
        ...
        case _EXIT_KEY_MESSAGE:
            OsdDispClearSliderAndNumber();
            SET_OSD_STATE(_MENU_OTHER);
            OsdDispClearSelectColor(_DOWN);// alant check
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_1);  //  OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_2);
            OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_OTHER_0);
            break;
...
void MenuDisplayFunDispRotateAdjust(void)
{
    ...
    switch(GET_KEYMESSAGE())
    {
        ...
        case _LEFT_KEY_MESSAGE:
            if((SysModeGetModeState() == _MODE_STATUS_DISPLAY_CHECK) &&
				  (SysRegionCheckAllDisplayState(_DISPLAY_STATE_NO_SIGNAL) != _TRUE) &&
				(GET_OSD_DISP_ROTATE() != _DISP_ROTATE_0))
            {
                if(GET_OSD_DISP_ROTATE() == (_DISP_ROTATE_0 + 4))
                {
                    OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_DISP_ROTATE_0);
...
void MenuColorColortempAdjust(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _LEFT_KEY_MESSAGE:
            ...
            {
                if(GET_COLOR_TEMP_TYPE(GET_OSD_SELECT_REGION()) == _CT_SRGB) // 12500k
                {
                    OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_TEMPRATURE_0);
                }
                OsdDispSliderAndNumber(_MENU_COLOR_TEMPERATURE_ADJUST, GET_COLOR_TEMP_TYPE(GET_OSD_SELECT_REGION()));
...
void MenuColorColorDemoAdjust(void)
{
    switch(GET_KEYMESSAGE())
    {
       ...
       //     OsdDispMainMenuIconPage(_UP, _ICON_PAGE_COLOR_0);//alant check
        ...
        case _EXIT_KEY_MESSAGE:
          ...
          //  OsdDispMainMenuIconPage(_UP, _ICON_PAGE_COLOR_1);//alant check
    ...
    switch(GET_KEYMESSAGE())
    {
        case _MENU_KEY_MESSAGE:
           ...
           if(g_usBackupValue != GET_OSD_ASPECT_RATIO_TYPE(UserAdjustGetSelectRegionPort()))
            ...
            SET_OSD_STATE(_MENU_ADVANCE_ASPECT);
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
            OsdDispMainMenuOptionSetting(_OPTION_ASPECT_RATIO_TYPE, _UNSELECT, g_usBackupValue);//GET_OSD_ASPECT_RATIO_TYPE(UserAdjustGetSelectRegionPort()));
            break;
...
void MenuOtherRotateAdjust(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _LEFT_KEY_MESSAGE:
           ...
           if(g_usAdjustValue != GET_OSD_STATUS())//  if(g_usAdjustValue != GET_OSD_ROTATE_STATUS())
            {

                 SET_OSD_STATUS(g_usAdjustValue);// SET_OSD_ROTATE_STATUS(g_usAdjustValue);
        ...
        case _EXIT_KEY_MESSAGE:
#if 0            
          if(g_usBackupValue != GET_OSD_STATUS())//   if(g_usBackupValue != GET_OSD_ROTATE_STATUS())
            {
                 g_usAdjustValue = g_usBackupValue;
...
void MenuOtherSelectRegionWidthAdjust(void)// usb power 
{
    switch(GET_KEYMESSAGE())
    {
        case _MENU_KEY_MESSAGE:
 #if 1    // alant debug       
            if(g_usBackupValue != GET_USB_POWER_STATUS())
            {
             //   SET_USB_POWER_STATUS(g_usAdjustValue);
                SET_OSD_EVENT_MESSAGE(_OSDEVENT_SAVE_NVRAM_OSDUSERDATA_MSG);
            }
...
void MenuColorColortempUserR(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _EXIT_KEY_MESSAGE:
            ...
            OsdDispClearSelectColor(_UP);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_CLEAR);
          OsdDispMainMenuItemSelection(_ITEM_1, _OSD_PRE_SELECT);//   OsdDispMainMenuItemSelection(_ITEM_2, _OSD_PRE_SELECT);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_COLOR_0);
...
void MenuColorColorEffcUserR(void)
{
    switch(GET_KEYMESSAGE())
    {
        ...
        case _EXIT_KEY_MESSAGE:
            ...
            OsdDispClearSelectColor(_UP);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_CLEAR);
               OsdDispMainMenuItemSelection(_ITEM_2, _OSD_PRE_SELECT);// OsdDispMainMenuItemSelection(_ITEM_3, _OSD_PRE_SELECT);
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_COLOR_0);
...
void MenuHotKeySource(void)
{
 ...
 #if 1   // alant debug        
            SET_OSD_1P_INPUT_SOURCE(g_usBackupValue);
           ScalerTimerReactiveTimerEvent(SEC(3), _USER_TIMER_EVENT_INPUT_SOURCE);
 #endif          
