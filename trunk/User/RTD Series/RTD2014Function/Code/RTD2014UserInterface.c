...
BYTE g_pucUartData[MAX_BUFF_SIZE];//BYTE g_pucUartData[5];
#endif
...
//BYTE ReadBuffer(char* buf);// alant add
...
void UserInterfaceTimerEventProc(EnumScalerTimerEventID enumEventID)
{
    switch(enumEventID)
    {
       case _USER_TIMER_EVENT_FACTORY_MODE:
        ...
        DebugMessageIR("factory mode",0);
#endif // End of #if(_DEBUG_MESSAGE_IR == _ON)
       g_stFacFlag=0;//reset
       break;
      case _USER_TIMER_EVENT_INPUT_SOURCE:
//#if(_DEBUG_MESSAGE_SUPPORT == _ON)
//			 ScalerDebugMessage("source timer", 0x01);
//#endif 
	  	OsdDispDisableOsd();
	  ...
	  case _USER_TIMER_EVENT_BURN_IN:// alant debug
	
	    if(GET_FACTORY_BURNIN() == _ON)
		{
             RTDFactoryBurnInPattern();
//#if(_DEBUG_MESSAGE_SUPPORT == _ON)
//			 ScalerDebugMessage("BURN IN", 0x01);
//#endif 
	  ...
	  break;
        ...
        case _USER_TIMER_EVENT_OSD_SHOW_MAIN_MENU:
            SET_OSD_EVENT_MESSAGE(_OSDEVENT_SHOW_MAINMENU_MSG);
            break;
        ...
        case _USER_TIMER_EVENT_OSD_DISABLE:
            OsdDispDisableOsd();
             if(SysRegionCheckAllDisplayState(_DISPLAY_STATE_NO_SIGNAL) == _TRUE)
               {
                    if(GET_FACTORY_BURNIN())// Wesley add
                     {
                         RTDFactoryBurnInPattern();
                      }
                    else
        ...
        case _USER_TIMER_EVENT_SIGNAL_STATUS:
               PCB_SIGNAL_STATUS(_TRUE);  ///Wesley_For 32_Signal Status_20171220
        break;
...
void UserInterfaceUartIntHandler_SERIALPORT(void)
{
  ...
  if(GET_UART_RECEIVE_STATUS() == _TRUE)
  {
	if ((gB_RcvStatus == RCV_EMPTY) &&	   // Packet not received yet
		 (gB_RcvCount < MAX_BUFF_SIZE))	 // Buffer not full
	{									   // Get data from UART
	 
		g_pucUartData[gB_RcvCount] = GET_UART_DATA();
		gB_RcvCount++;
...
void UserInterfaceInitial(void)
{
    ...
    PCB_PWM_SETTING();
    RTDNVRamStartup(); // load user data from nvram
...
void UserInterfacePowerSwitch(EnumPowerAction enumSwitch)
{
    switch(enumSwitch)
    {
        ...
        case _POWER_ACTION_PS_TO_NORMAL:// wake up
			
			//PCB_USB_POWER(GET_USB_POWER_STATUS());// alant debug

            break;

        case _POWER_ACTION_OFF_TO_NORMAL: // DC off-> DC on 
            ...
            SET_FACTORY_BURNIN(_OFF);//alant check
    ...
    switch(ucKeyPadLed)
    {
     default:
        ...
        case _LED_POWER_ACTIVE:
            PCB_LED_ON();//PCB_LED_ACTIVE();
            break;

        case _LED_POWER_SEARCH:
        PCB_LED_ON();//     PCB_LED_IDLE();
            break;

        case _LED_POWER_SAVING:
           PCB_LED_OFF();//   PCB_LED_POWER_SAVING();
            break;
/*
...
{
    enumDisplayRegion = enumDisplayRegion;
//printf("UserInterfaceGetRGBQuantizationLimitRangeStatus =%bd\r\n", enumColorRGBQuantization);
