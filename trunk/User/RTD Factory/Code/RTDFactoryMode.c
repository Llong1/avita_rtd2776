...
void FactoryReset(void)
{
  //   BYTE u1model = GET_MODEL_TYPE();
	...
	//   RTDNVRamLoadColorSetting(GET_OSD_SELECT_REGION());
       ...
       SET_OSD_LANGUAGE(0);// english
	   SET_FACTORY_BURNIN(_OFF);// alant debug

       //   SET_MODEL_TYPE(u1model);
...
BYTE u1rotate =0;
   ...
   u1rotate= GET_OSD_ROTATE_STATUS();
   ...
   OsdDispOsdRotateSwitch();
    ...
    ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_GO_TO_POWER_SAVING); // alant debug
...
//Wesley Factory Displey
    ...
    RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_PANEL_NAME_ROW), COL(_FACTORY_FUNCTION_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[1], _ENGLISH);
    RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_FW_VERSION_ROW), COL(_FACTORY_FUNCTION_COL + 16), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[2], _ENGLISH);
    ///RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_MODEL_NAME_ROW), COL(_FACTORY_FUNCTION_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[0], _ENGLISH);
...
//alant:  factory reset
 ...
 //   RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_BURNIN_TIME_ROW), COL(_FACTORY_FUNCTION_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[6], _ENGLISH);
    ...
    // Burnin on or off
    RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_BURNIN_ROW), COL(_FACTORY_ON_OFF_COL), _PFONT_FACTORY_BURNIN, tFONT_FACTORY_EUROPE, 
    tOSD_FACTORY_ON_OFF_TABLE[GET_FACTORY_BURNIN()], _ENGLISH);

    // Panel/Burnin Time hour and min.
     for(ucRowCount = 1; ucRowCount < 2; ucRowCount++)//for(ucRowCount = 0; ucRowCount < 2; ucRowCount++)
    {
 ...
 //   RTDFactoryOsdFuncShowNumber(ROW(_FACTORY_BURNIN_TIME_ROW), COL(_FACTORY_BURNIN_TIME_HOUR_NUMBER_COL), 0, (_ALIGN_LEFT | _FORCE_SHOW_NUMBER_OFF | _SHOW_5), _PFONT_FACTORY_BURNIN_TIME_HOUR);
  //  RTDFactoryOsdFuncShowNumber(ROW(_FACTORY_BURNIN_TIME_ROW), COL(_FACTORY_BURNIN_TIME_MIN_NUMBER_COL), 0, (_ALIGN_LEFT | _FORCE_SHOW_NUMBER_OFF | _SHOW_3), _PFONT_FACTORY_BURNIN_TIME_MIN);
    ...
    RTDFactoryOsdFuncEnableOsd();
     ...
     SET_OSD_ROTATE_STATUS(u1rotate);
}
...
{
  //  g_usAdjustValue = 0;
      g_usAdjustValue = GET_FACTORY_BURNIN();
...
//============= Reset Burnin Time =======================
void RTDFactoryResetBurninTimeEnter(void)// factory reset
{
    FactoryReset();
}
...
//============= Exit=======================
void RTDFactoryExitEnter(void)// press exit option
{
    RTDFactoryOsdFuncDisableOsd();
...
void RTDFactoryExitExit(void)// press back key
{
    RTDFactoryOsdFuncDisableOsd();
...
void RTDFactoryBurninOnOffEnter(void)
{
    ...
    g_usAdjustValue= GET_FACTORY_BURNIN();// alant debug
	      ...
	      RTDNVRamSaveOsdUserData();
	   	    ...
	   	    if(GET_FACTORY_BURNIN())
	          {
		       RTDFactoryBurnInPattern();
                  ...
                  {
                     ScalerDDomainBackgroundEnable(_DISPLAY_PATH_ALL, _TRUE);
