...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerTMDSRx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __SCALER_TMDSRX0__
...
WORD g_usTMDSRx0ClkCount;

BYTE g_pucTMDSRx0LEMax[3];
BYTE g_pucTMDSRx0LEMin[3];

#if(_D0_INPUT_PORT_TYPE == _D0_MHL_PORT)
BYTE g_ucTMDSRx0ClkMode = _TMDS_24BIT_PHY_SETTING;
#endif

#if(_D0_HDMI_SUPPORT == _ON)
BYTE g_ucTMDSRx0AviInfo;
#endif

EnumDisplayDataPath g_enumTMDSRx0IrqForceBackgrd;

//****************************************************************************
// FUNCTION DECLARATIONS
//****************************************************************************
void ScalerTMDSRx0Initial(BYTE ucInputPort);
void ScalerTMDSRx0InterruptInitial(void);
void ScalerTMDSRx0SetDFEInitial_EXINT0(void);
void ScalerTMDSRx0SetPhy_EXINT0(void);
void ScalerTMDSRx0IntHandler_EXINT0(void);
bit ScalerTMDSRx0CheckCDRStable(WORD usClkCount);
BYTE ScalerTMDSRx0CheckCDRStable_EXINT0(WORD usClkCount);
WORD ScalerTMDSRx0MeasureClk(BYTE ucMeasureClk);
WORD ScalerTMDSRx0MeasureClk_EXINT0(BYTE ucMeasureClk);
void ScalerTMDSRx0DviVideoSetting(void);
void ScalerTMDSRx0SetPorchColor(EnumColorSpace enumColorFormat);

#if(_D0_HDMI_SUPPORT == _ON)
bit ScalerTMDSRx0HdmiDeepColorProc(WORD usTmdsClkCnt);
bit ScalerTMDSRx0HdmiSetDeepColorPll(WORD usTMDSClkCount, BYTE ucColorDepth);
bit ScalerTMDSRx0HdmiWaitDeepColorPllLock(void);
void ScalerTMDSRx0HdmiPacketDetect(bit bWait);
void ScalerTMDSRx0HdmiPacketDetectEvent(void);
BYTE ScalerTMDSRx0HdmiGetAVIInfo(EnumAviInfoContent enumContent);
bit ScalerTMDSRx0HdmiAviPixelRepInfoChanged(void);
BYTE ScalerTMDSRx0HdmiGetColorDepth(void);
void ScalerTMDSRx0HdmiAVMuteProc(void);
void ScalerTMDSRx0HdmiWatchDog(BYTE ucWDType, bit bEnable);

#if(_D0_HDMI_2_0_SUPPORT == _ON)
void ScalerTMDSRx0PSIntHandler_EXINT0(void);
void ScalerTMDSRx0ReadRequestProc_EXINT0(BYTE ucRROutCome);
void ScalerTMDSRx0HDMI2HPDRRIRQEnable(bit bEnable);
#endif

#if(_HDMI_FREESYNC_SUPPORT == _ON)
BYTE ScalerTMDSRx0HdmiGetAmdSpdInfo(EnumAmdSpdInfoContent enumContent);
#endif
#endif

void ScalerTMDSRx0PowerSavingProc(void);
void ScalerTMDSRx0PowerOffProc(void);
void ScalerTMDSRx0FreqDetectDisable(void);
void ScalerTMDSRx0FirstActiveProc(BYTE ucSourceType);
void ScalerTMDSRx0TMDSScanInitial(BYTE ucInputPort);
bit ScalerTMDSRx0TMDSPSPreDetect(BYTE ucInputPort);
bit ScalerTMDSRx0TMDSNormalPreDetect(BYTE ucInputPort);
BYTE ScalerTMDSRx0TMDSScanInputPort(BYTE ucInputType);
BYTE ScalerTMDSRx0FormatDetect(void);
bit ScalerTMDSRx0HdcpEnabled(void);
BYTE ScalerTMDSRx0TMDSVideoDetect(bit bWait);
void ScalerTMDSRx0TMDSVideoDetectEvent(void);
bit ScalerTMDSRx0TMDSStableDetect(BYTE ucSourceType);
void ScalerTMDSRx0HotPlugEvent(void);
...
void ScalerTMDSRx0Initial(BYTE ucInputPort)
{
    ...
    if(ScalerGetBit(P2_E6_HDMI_FREQDET_CTRL, _BIT1) == 0x00)
    {
        ...
        CLR_TMDS_RX0_HIGH_SPEED_MODE();
    }

    // Disable SCDC Scrambling Status
    ScalerSetDataPortBit(P2_39_HDMI_CTRL_39, _P2_3A_PT_21_PORT_PAGE2_HDMI_SCDC_21, ~_BIT0, 0x00);
    ...
    // Set D0 Port HDCP Address is 0x74
...
#else // Else of #if(_D0_HDMI_SUPPORT == _ON)
    ...
    ScalerSetBit(P2_C0_HDCP_CR, ~(_BIT7 | _BIT0), (_BIT7 | _BIT0));
    ...
    // Set MAC Signal Swap
    ScalerSetBit(P2_A7_TMDS_PWDCTL, ~(_BIT6 | _BIT5), ((_D0_TMDS_BR_SWAP << 6) | (_D0_TMDS_PN_SWAP << 5)));
    ...
    if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        ...
        ScalerSetBit(P26_A0_MHL_CTRL_00, ~(_BIT5 | _BIT0), ((_D0_TMDS_PN_SWAP << 5) | _BIT0));
...
void ScalerTMDSRx0InterruptInitial(void)
{
    ...
    if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        ...
        ScalerSetBit(P2_E7_HDMI_FREQDET_OFFSET, ~(_BIT2 | _BIT1 | _BIT0), _BIT2);
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0SetDFEInitial_EXINT0(void) using 1
{
    ...
    if((GET_D0_INPUT_PIXEL_CLK() > 1000) && ((GET_D0_INPUT_PIXEL_CLK() < 3400)))
    {
    ...
    ScalerSetBit_EXINT(P1D_F8_GRAY_DEC_2, ~(_BIT5 | _BIT4), (_BIT5 | _BIT4));
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() < 700)
    {
        // LE Max = 24, Initial = 6
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() < 1000)
    {
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() < 3400)
    {
    ...
    if((GET_D0_INPUT_PIXEL_CLK() > 1000) && (GET_D0_INPUT_PIXEL_CLK() < 3400))
    {
        ...
        ScalerSetByte_EXINT(P1D_A5_L0_INIT_3, 0x0C);
        ScalerSetByte_EXINT(P1D_B5_L1_INIT_3, 0x0C);
        ScalerSetByte_EXINT(P1D_C5_L2_INIT_3, 0x0C);
    }
    else
    {
    ...
    if(GET_D0_INPUT_PIXEL_CLK() < 1500)
    {
        // Vth Initial Value = 6
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0SetPhy_EXINT0(void) using 1
{
    BYTE ucTemp = 0;
    ...
    ScalerSetBit_EXINT(PB_A1_CMU_01, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);

    if(ScalerGetBit_EXINT(PB_A0_CMU_00, _BIT6) == _BIT6)  // MHL Mode
    {
        // [7:4] rx_en = 4'b1001 --> Enable RXEN for Lane0, 3
        ScalerSetBit_EXINT(PB_A1_CMU_01, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT4));
    ...
    if(ScalerGetBit_EXINT(PB_A0_CMU_00, _BIT6) == _BIT6)   // MHL Mode
    {
        ...
        else  // MHL 24bit Mode
        {
            ...
            // [3:1] CMU_SEL_RI = 3'b011 --> Rs = 12KOhm
            // [0] CMU_SEL_CP = 1'b1 --> Cp = 2pF
            ScalerSetByte_EXINT(PB_A4_CMU_04, 0x47);

            if((GET_D0_INPUT_PIXEL_CLK() >= 700) && (GET_D0_INPUT_PIXEL_CLK() < 1125))
            {
                // [7:5] rate_sel = 3'b010 --> 750MHz ~ 1125MHz
                ScalerSetBit_EXINT(PB_DB_ST_0, ~(_BIT7 | _BIT6 | _BIT5), _BIT6);

                // [7:5] SEL_DIV = 3'b010 --> 750MHz ~ 1125MHz
                // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
                // [1:0] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x50);

                // [7:0] N_CODE = 8'b0011 1000 --> 56 + 4 = 60
                ScalerSetByte_EXINT(PB_F2_SDM_01, 0x38);
            }
            else if((GET_D0_INPUT_PIXEL_CLK() >= 1125) && (GET_D0_INPUT_PIXEL_CLK() < 1500))
            {
                ...
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x51);

                // [7:0] N_CODE = 8'b0111 0100 --> 116 + 4 = 120
                ScalerSetByte_EXINT(PB_F2_SDM_01, 0x74);
            }
            else if((GET_D0_INPUT_PIXEL_CLK() >= 1500) && (GET_D0_INPUT_PIXEL_CLK() < 2360))
            {
                ...
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x32);
    ...
    {
#if(_D0_HDMI_2_0_SUPPORT == _ON)
        if(GET_TMDS_RX0_HIGH_SPEED_MODE() == _TRUE)
        {
            ...
            // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
            // [1:0] CMU_SELPREDIV = 2'b00 --> /1
            ScalerSetByte_EXINT(PB_A3_CMU_03, 0x10);

            // [7:4] CMU_SEL_PUMP_I = 4'b0101 --> Icp = 15uA
            // [3:1] CMU_SEL_RI = 3'b011 --> Rs = 12KOhm
            // [0] CMU_SEL_CP = 1'b1 --> Cp = 2pF
            ScalerSetByte_EXINT(PB_A4_CMU_04, 0x57);
...
#endif
        {
            ...
            // [3:1] CMU_SEL_RI = 3'b011 --> Rs = 12KOhm
            // [0] CMU_SEL_CP = 1'b1 --> Cp = 2pF
            ScalerSetByte_EXINT(PB_A4_CMU_04, 0xA6);

            if((GET_D0_INPUT_PIXEL_CLK() >= 187) && (GET_D0_INPUT_PIXEL_CLK() < 375))
            {
                // [7:5] rate_sel = 3'b100 --> 187.5MHz ~ 375MHz
                ScalerSetBit_EXINT(PB_DB_ST_0, ~(_BIT7 | _BIT6 | _BIT5), _BIT7);

                // [7:5] SEL_DIV = 3'b100 --> 187.5MHz ~ 375MHz
                // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
                // [1:0] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x90);

                // [7:0] N_CODE = 8'b0100 1100 --> 76 + 4 = 80
                ScalerSetByte_EXINT(PB_F2_SDM_01, 0x4C);
            }
            else if((GET_D0_INPUT_PIXEL_CLK() >= 375) && (GET_D0_INPUT_PIXEL_CLK() < 750))
            {
                // [7:5] rate_sel = 3'b011 --> 375MHz ~ 750MHz
                ScalerSetBit_EXINT(PB_DB_ST_0, ~(_BIT7 | _BIT6 | _BIT5), (_BIT6 | _BIT5));

                // [7:5] SEL_DIV = 3'b011 --> 375MHz ~ 750MHz
                // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
                // [1:0] CMU_SELPREDIV = 2'b01 --> /2
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x71);

                // [7:0] N_CODE = 8'b0100 1100 --> 76 + 4 = 80
                ScalerSetByte_EXINT(PB_F2_SDM_01, 0x4C);
            }
            else if((GET_D0_INPUT_PIXEL_CLK() >= 750) && (GET_D0_INPUT_PIXEL_CLK() < 1500))
            {
            ...
            else if((GET_D0_INPUT_PIXEL_CLK() >= 1500) && (GET_D0_INPUT_PIXEL_CLK() < 3000))
            {
            ...
            else if((GET_D0_INPUT_PIXEL_CLK() >= 3000) && (GET_D0_INPUT_PIXEL_CLK() < 3400))
            {
                ...
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x13);

                // [4:1] CMU_PREDIVN = 4'b0110 --> /8 (N+2 Divider)
                ScalerSetBit_EXINT(PB_A8_CMU_08, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1), (_BIT3 | _BIT2));

                // [7:0] N_CODE = 8'b0010 1000 --> 36 + 4 = 40
                ScalerSetByte_EXINT(PB_F2_SDM_01, 0x24);
            }
            else if((GET_D0_INPUT_PIXEL_CLK() >= 3400) && (GET_D0_INPUT_PIXEL_CLK() < 6300))
            {
                // [7:5] rate_sel = 3'b000 --> 3400MHz ~ 6000MHz
                ScalerSetBit_EXINT(PB_DB_ST_0, ~(_BIT7 | _BIT6 | _BIT5), 0x00);
                ...
                // [0] CMU_SEL_CP = 1'b1 --> Cp = 2pF
                ScalerSetByte_EXINT(PB_A4_CMU_04, 0x55);
    ...
    ScalerSetBit_EXINT(PB_A5_CMU_05, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT5 | _BIT3));

    // [7] BYPASS_PI = 1'b1 --> Use Integer N_CODE
    // [5] N_CODE[8] = 1'b0 --> N = N_CODE + 4
    ScalerSetBit_EXINT(PB_F1_SDM_00, ~(_BIT7 | _BIT5), _BIT7);

    // [0] PLL_WD_EN = 1'b0 --> Disable WD
    ScalerSetBit_EXINT(PB_F4_WD_00, ~_BIT0, 0x00);
    ...
    if((GET_D0_INPUT_PIXEL_CLK() >= 2500) && (GET_D0_INPUT_PIXEL_CLK() < 3000))
    {
        // [3:0] KD = 4'b1111 --> KD = 1
        ScalerSetBit_EXINT(PB_CC_ERROR_COUNT_0, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT2 | _BIT1 | _BIT0));
    }
    else
    {
        // [3:0] KD = 4'b0000 --> KD = 0
    ...
    if(ScalerGetBit_EXINT(PB_A0_CMU_00, _BIT6) == _BIT6) // MHL Mode
    {
        ...
        {
            // KI = 3'b010 --> KI = 1/8
            ScalerSetBit_EXINT(PB_D4_KI_0, ~(_BIT7 | _BIT6 | _BIT5), _BIT6);

            // KP = 0x33 --> KP = 12.75
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x33);
        }
        else
        {
            // KI = 3'b000 --> KI = 0
            ScalerSetBit_EXINT(PB_D4_KI_0, ~(_BIT7 | _BIT6 | _BIT5), 0x00);

            // KP = 0x2F --> KP = 11.75
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x2F);
        }
    }
    else   // HDMI Mode
#endif
    {
        ...
        if(GET_D0_INPUT_PIXEL_CLK() < 2500)
        {
            // KP = 0x0F --> KP = 3.75
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x0F);
            ScalerSetByte_EXINT(PB_D7_KP_1, 0x0F);
            ScalerSetByte_EXINT(PB_D8_KP_2, 0x0F);
        }
        else if((GET_D0_INPUT_PIXEL_CLK() >= 2500) && (GET_D0_INPUT_PIXEL_CLK() < 3000))
        {
            // KP = 0x2D --> KP = 11.25
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x2D);
            ScalerSetByte_EXINT(PB_D7_KP_1, 0x2D);
            ScalerSetByte_EXINT(PB_D8_KP_2, 0x2D);
        }
        else if(GET_D0_INPUT_PIXEL_CLK() < 3400)
        {
            // KP = 0x17 --> KP = 5.75
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x17);
            ScalerSetByte_EXINT(PB_D7_KP_1, 0x17);
            ScalerSetByte_EXINT(PB_D8_KP_2, 0x17);
        }
        else
        {
            ...
            ScalerSetBit_EXINT(PB_FE_LANE0_A_CDR_08, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT1);
    ...
    ScalerTMDSRx0SetDFEInitial_EXINT0();

    // [7:5] LE_lowgain_highboost = 1 --> LE HighBoost Mode
    ScalerSetBit_EXINT(PB_A8_CMU_08, ~(_BIT7 | _BIT6 | _BIT5), _BIT7);

    // [0] Tap2~6 Adaptation Revise
    ScalerSetBit_EXINT(PB_B2_EQ_00, ~_BIT0, 0x00);

    if(GET_D0_INPUT_PIXEL_CLK() < 2000)
    {
        // [4] SEL_DFE_DELAY = 1'b1 --> Increase DFE Delay
        ScalerSetBit_EXINT(PB_A6_CMU_06, ~_BIT4, _BIT4);
    }
    else
    {
        // [4] SEL_DFE_DELAY = 1'b0 --> Normal DFE Delay
        ScalerSetBit_EXINT(PB_A6_CMU_06, ~_BIT4, 0x00);
    }
    ...
    else if((GET_D0_INPUT_PIXEL_CLK() >= 2500) && (GET_D0_INPUT_PIXEL_CLK() < 3400))
    {
        // [7:5] LE_ISEL = 3'b010
        // [4:2] LE_RLSEL = 3'b001
    ...
    ScalerSetBit_EXINT(PB_B0_CDR_07, ~_BIT1, _BIT1);
    ...
    // Enable Tap1~4
    ScalerSetBit_EXINT(PB_B2_EQ_00, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT5 | _BIT4 | _BIT3 | _BIT2));

    // Enable Adaptation
    if(GET_D0_INPUT_PIXEL_CLK() > 3400)  // 3400~6000 Analog CDR
    {
        ...
        ScalerSetBit_EXINT(PB_F9_LANE0_A_CDR_03, ~(_BIT6 | _BIT5 | _BIT4), _BIT4);

        // [0] en_wide_temp = 1'b1
        ScalerSetBit_EXINT(PB_AB_CDR_02, ~_BIT0, _BIT0);
        ...
        if(GET_TMDS_RX0_HIGH_SPEED_MODE() == _TRUE)
        {
            ...
            ScalerSetByte_EXINT(PB_FB_LANE0_A_CDR_05, 0x3E);
        ...
        {
            // Analog CDR Auto FLD Counter
            ScalerSetByte_EXINT(PB_FA_LANE0_A_CDR_04, 0xA0);
            ScalerSetByte_EXINT(PB_FB_LANE0_A_CDR_05, 0xA0);
            ScalerSetByte_EXINT(PB_FC_LANE0_A_CDR_06, 0x05);
        }
        ...
        // Read Back Vth
        ScalerSetByte_EXINT(P1D_F0_COEF_1, 0x00);

        // Reload Vth - 1
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT7 | _BIT6), 0x00);
        ScalerSetByte_EXINT(P1D_A9_L0_INIT_7, (ScalerGetByte_EXINT(P1D_F1_COEF_2) & 0x0F) - 1);

        // Reload Vth
        ScalerSetByte_EXINT(P1D_AA_L0_INIT_8, 0x40);
        ScalerSetByte_EXINT(P1D_AA_L0_INIT_8, 0x00);
        ...
        g_pucTMDSRx0LEMin[0] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        // Read Back LE Max
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), _BIT3);

        g_pucTMDSRx0LEMax[0] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        if(g_pucTMDSRx0LEMax[0] < 10)
        {
            g_pucTMDSRx0LEMax[0]++;
        }
        else if(g_pucTMDSRx0LEMax[0] > 12)
        {
            g_pucTMDSRx0LEMax[0]--;
        }

        // Read Max Value of LE
        ScalerSetBit_EXINT(P1D_A2_L0_LIMIT_INIT, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx0LEMax[0] & (_BIT3 | _BIT2 | _BIT1 | _BIT0)));
        ScalerSetBit_EXINT(P1D_A3_L0_INIT_1, ~_BIT6, ((g_pucTMDSRx0LEMax[0] << 2) & _BIT6));
        ...
        ScalerSetBit_EXINT(P1D_AC_L0_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetByte_EXINT(P1D_A1_L0_DFE_EN_2, 0x03);
        ...
        ScalerSetByte_EXINT(P1D_A5_L0_INIT_3, ScalerGetByte_EXINT(P1D_F3_COEF_4));

        // Reload Max Value of Tap0~1
        ScalerSetBit_EXINT(P1D_AA_L0_INIT_8, ~(_BIT1 | _BIT0), (_BIT1 | _BIT0));
        ScalerSetBit_EXINT(P1D_AA_L0_INIT_8, ~(_BIT1 | _BIT0), 0x00);

        // Enable Tap3~4 Adaptation
        ScalerSetBit_EXINT(P1D_AC_L0_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetByte_EXINT(P1D_A1_L0_DFE_EN_2, 0x18);
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() > 2900) // 2900~3400
    {
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() > 375) // 375~2900
    {
        // Disable Analog CDR Mode
        ScalerSetBit_EXINT(PB_B9_LANE0_A_CDR_00, ~_BIT7, 0x00);
    ...
    if(GET_D0_INPUT_PIXEL_CLK() > 3400) // 3400~6000 Analog CDR
    {
        ...
        // Reload Vth - 1
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT6);
        ScalerSetByte_EXINT(P1D_B9_L1_INIT_7, (ScalerGetByte_EXINT(P1D_F1_COEF_2) & 0x0F) - 1);

        // Reload Vth - 1
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
        ScalerSetByte_EXINT(P1D_C9_L2_INIT_7, (ScalerGetByte_EXINT(P1D_F1_COEF_2) & 0x0F) - 1);

        // Reload Vth
        ScalerSetByte_EXINT(P1D_BA_L1_INIT_8, 0x40);
        ScalerSetByte_EXINT(P1D_CA_L2_INIT_8, 0x40);
        ScalerSetByte_EXINT(P1D_BA_L1_INIT_8, 0x00);
        ScalerSetByte_EXINT(P1D_CA_L2_INIT_8, 0x00);
        ...
        g_pucTMDSRx0LEMin[1] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        // Read Back LE Max
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), _BIT3);

        g_pucTMDSRx0LEMax[1] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        if(g_pucTMDSRx0LEMax[1] < 10)
        {
            g_pucTMDSRx0LEMax[1]++;
        }
        else if(g_pucTMDSRx0LEMax[1] > 12)
        {
            g_pucTMDSRx0LEMax[1]--;
        }

        // Read Max Value of LE
        ScalerSetBit_EXINT(P1D_B2_L1_LIMIT_INIT, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx0LEMax[1] & (_BIT3 | _BIT2 | _BIT1 | _BIT0)));
        ScalerSetBit_EXINT(P1D_B3_L1_INIT_1, ~_BIT6, ((g_pucTMDSRx0LEMax[1] << 2) & _BIT6));
        ...
        g_pucTMDSRx0LEMin[2] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        // Read Back LE Max
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), _BIT3);

        g_pucTMDSRx0LEMax[2] = ScalerGetByte_EXINT(P1D_F3_COEF_4);

        if(g_pucTMDSRx0LEMax[2] < 10)
        {
            g_pucTMDSRx0LEMax[2]++;
        }
        else if(g_pucTMDSRx0LEMax[2] > 12)
        {
            g_pucTMDSRx0LEMax[2]--;
        }

        // Read Max Value of LE
        ScalerSetBit_EXINT(P1D_C2_L2_LIMIT_INIT, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx0LEMax[2] & (_BIT3 | _BIT2 | _BIT1 | _BIT0)));
        ScalerSetBit_EXINT(P1D_C3_L2_INIT_1, ~_BIT6, ((g_pucTMDSRx0LEMax[2] << 2) & _BIT6));
        ...
        ScalerSetBit_EXINT(P1D_BC_L1_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P1D_CC_L2_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetByte_EXINT(P1D_B1_L1_DFE_EN_2, 0x03);
        ScalerSetByte_EXINT(P1D_C1_L2_DFE_EN_2, 0x03);
        ...
        // Read Max Value of Tap1
        ScalerSetByte_EXINT(P1D_B5_L1_INIT_3, ScalerGetByte_EXINT(P1D_F3_COEF_4));

        // Read Back Lane2
        ScalerSetBit_EXINT(P1D_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);

        // Read Max Value of Tap1
        ScalerSetByte_EXINT(P1D_C5_L2_INIT_3, ScalerGetByte_EXINT(P1D_F3_COEF_4));

        // Reload Max Value of Tap0~1
        ScalerSetBit_EXINT(P1D_BA_L1_INIT_8, ~(_BIT1 | _BIT0), (_BIT1 | _BIT0));
        ScalerSetBit_EXINT(P1D_CA_L2_INIT_8, ~(_BIT1 | _BIT0), (_BIT1 | _BIT0));
        ScalerSetBit_EXINT(P1D_BA_L1_INIT_8, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P1D_CA_L2_INIT_8, ~(_BIT1 | _BIT0), 0x00);

        // Enable Tap3~4 Adaptation
        ScalerSetBit_EXINT(P1D_BC_L1_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P1D_CC_L2_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetByte_EXINT(P1D_B1_L1_DFE_EN_2, 0x18);
        ScalerSetByte_EXINT(P1D_C1_L2_DFE_EN_2, 0x18);
    ...
    else if(GET_D0_INPUT_PIXEL_CLK() > 2900) // 2900~3400
    {
        ...
        ScalerSetBit_EXINT(P1D_BC_L1_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P1D_CC_L2_DFE_EN_3, ~(_BIT1 | _BIT0), 0x00);
        ScalerSetByte_EXINT(P1D_A1_L0_DFE_EN_2, 0x06);
        ScalerSetByte_EXINT(P1D_B1_L1_DFE_EN_2, 0x06);
        ScalerSetByte_EXINT(P1D_C1_L2_DFE_EN_2, 0x06);
        ...
        // Lane0 LEQ >= 14
        if(g_pucTMDSRx0LEMax[0] >= 14)
        {
            // Set Tap2 = 6
            ScalerSetByte_EXINT(P1D_A6_L0_INIT_4, 0x06);

            if(g_pucTMDSRx0LEMax[0] < 16)
            {
                g_pucTMDSRx0LEMax[0] = g_pucTMDSRx0LEMax[0] + 2;
            }
            else if(g_pucTMDSRx0LEMax[0] > 18)
            {
                g_pucTMDSRx0LEMax[0] = g_pucTMDSRx0LEMax[0] - 2;
            }

            // Set Max Value of LE
            ScalerSetBit_EXINT(P1D_A5_L0_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), g_pucTMDSRx0LEMax[0]);

            // Reload LE/Tap2
            ScalerSetBit_EXINT(P1D_AA_L0_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
            ScalerSetBit_EXINT(P1D_AA_L0_INIT_8, ~(_BIT2 | _BIT1), 0x00);
        }

        // Lane1 LEQ >= 14
        if(g_pucTMDSRx0LEMax[1] >= 14)
        {
            // Set Tap2 = 6
            ScalerSetByte_EXINT(P1D_B6_L1_INIT_4, 0x06);

            if(g_pucTMDSRx0LEMax[1] < 16)
            {
                g_pucTMDSRx0LEMax[1] = g_pucTMDSRx0LEMax[1] + 2;
            }
            else if(g_pucTMDSRx0LEMax[1] > 18)
            {
                g_pucTMDSRx0LEMax[1] = g_pucTMDSRx0LEMax[1] - 2;
            }

            // Reload Max Value of LE
            ScalerSetBit_EXINT(P1D_B5_L1_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), g_pucTMDSRx0LEMax[1]);

            // Reload LE/Tap2
            ScalerSetBit_EXINT(P1D_BA_L1_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
            ScalerSetBit_EXINT(P1D_BA_L1_INIT_8, ~(_BIT2 | _BIT1), 0x00);
        }

        // Lane2 LEQ >= 14
        if(g_pucTMDSRx0LEMax[2] >= 14)
        {
            // Set Tap2 = 6
            ScalerSetByte_EXINT(P1D_C6_L2_INIT_4, 0x06);

            if(g_pucTMDSRx0LEMax[2] < 16)
            {
                g_pucTMDSRx0LEMax[2] = g_pucTMDSRx0LEMax[2] + 2;
            }
            else if(g_pucTMDSRx0LEMax[2] > 18)
            {
                g_pucTMDSRx0LEMax[2] = g_pucTMDSRx0LEMax[2] - 2;
            }

            // Reload Max Value of LE
            ScalerSetBit_EXINT(P1D_C5_L2_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), g_pucTMDSRx0LEMax[2]);

            // Reload LE/Tap2
            ScalerSetBit_EXINT(P1D_CA_L2_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
            ScalerSetBit_EXINT(P1D_CA_L2_INIT_8, ~(_BIT2 | _BIT1), 0x00);
        }
    ...
    if(GET_D0_INPUT_PIXEL_CLK() > 3400) // 3400~6000 Hybrid CDR
    {
        // [0] en_eqen_manual = 0 --> Disable Adaptation
        ScalerSetBit_EXINT(PB_F6_EQEN_TIMER, ~_BIT0, 0x00);
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0IntHandler_EXINT0(void) using 1
{
#if(_D0_HDMI_2_0_SUPPORT == _ON)
    BYTE pucBackUp[4];
#else
    ...
    if((ScalerGetByte_EXINT(P2_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0)
    {
        ...
        ScalerSetByte_EXINT(P2_39_HDMI_CTRL_39, _P2_3A_PT_40_PORT_PAGE2_HDMI_SCDC_40);
        ...
        ScalerSetByte_EXINT(P2_39_HDMI_CTRL_39, _P2_3A_PT_10_PORT_PAGE2_HDMI_SCDC_10);
        ...
        SET_D0_INPUT_PIXEL_CLK((((DWORD)_GDI_CLK_KHZ * usCount + 50000) / 100000));

#if(_D0_HDMI_2_0_SUPPORT == _ON)

        // If SCDC tell 1:40 then set High speed flag
        if((GET_D0_INPUT_PIXEL_CLK() >= 850) && (GET_D0_INPUT_PIXEL_CLK() <= 1500))
        {
            ScalerSetByte_EXINT(P2_39_HDMI_CTRL_39, _P2_3A_PT_20_PORT_PAGE2_HDMI_SCDC_20);

            if(ScalerGetBit_EXINT(P2_3A_HDMI_CTRL_3A, _BIT1) == _BIT1)
            {
                SET_TMDS_RX0_HIGH_SPEED_MODE();
            }
        }

        if(GET_TMDS_RX0_HIGH_SPEED_MODE() == _TRUE)
        {
            SET_D0_INPUT_PIXEL_CLK((GET_D0_INPUT_PIXEL_CLK() * 4));
        }
#endif

        if((GET_TMDS_RX0_PHY_SET() == _FALSE) || ((bit)ScalerTMDSRx0CheckCDRStable_EXINT0(g_usTMDSRx0ClkCount) == _FALSE))
        {
            g_usTMDSRx0ClkCount = ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_TMDS);

            // TMDS_OFFLine Measure FREQ(MHz) = GDI_CLK(MHz) * 1024 / g_usTMDSRx0ClkCount
            SET_TMDS_D0_OFFLINE_MEASURE_CLK(((DWORD)(_GDI_CLK_KHZ * 1024) / 100) / g_usTMDSRx0ClkCount);

#if(_D0_HDMI_2_0_SUPPORT == _ON)
            if(GET_TMDS_RX0_HIGH_SPEED_MODE() == _TRUE)
            {
                SET_TMDS_D0_OFFLINE_MEASURE_CLK((GET_TMDS_D0_OFFLINE_MEASURE_CLK() * 4));
            }
#endif
            ...
            if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
            {
                ...
                // Check CDR Stable
                if((ScalerTMDSRx0CheckCDRStable_EXINT0(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerMHLRx0DetectClkMode_EXINT0() == _FALSE))// MHL mode need modify
                {
                    SET_TMDS_RX0_CLK_MODE(_TMDS_24BIT_PHY_SETTING);

                    // TMDS_FREQ(MHz) = GDI_CLK(MHz) * 1024 / CLK_COUNT = GDI_CLK(KHz) * 1024 / (CLK_COUNT * 1024)
                    SET_D0_INPUT_PIXEL_CLK(GET_D0_INPUT_PIXEL_CLK() * 3);
                    ScalerTMDSRx0SetPhy_EXINT0();
                    SET_D0_INPUT_PIXEL_CLK(GET_D0_INPUT_PIXEL_CLK() / 3);
                    ...
                    if((ScalerTMDSRx0CheckCDRStable_EXINT0(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerMHLRx0DetectClkMode_EXINT0() == _FALSE))// MHL Need modify
                    {
                        bMHLFail = _TRUE;
                    }
                }
...
#endif
        {
            ...
            SET_COLOR_DEPTH(_D0_INPUT_PORT, _COLOR_DEPTH_8_BITS);

            SET_TMDS_RX0_PHY_SET();
        }
    }
    ...
    if((ScalerGetByte_EXINT(P2_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06)
    {
        ...
        if((abs(g_usTMDSRx0ClkCount - ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_TMDS)) > (g_usTMDSRx0ClkCount >> 6)) ||
           (abs(GET_D0_INPUT_PIXEL_CLK() - ((GET_TMDS_D0_OFFLINE_MEASURE_CLK() << 3) / ((GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR422) ? _COLOR_DEPTH_8_BITS : GET_COLOR_DEPTH(_D0_INPUT_PORT)))) > (GET_D0_INPUT_PIXEL_CLK() >> 6)))
        {
            if((GET_TMDS_RX0_FORCE_BCKGRD_PATH() & _DISPLAY_PATH_M1) != 0)
            {
            ...
            if((GET_TMDS_RX0_FORCE_BCKGRD_PATH() & _DISPLAY_PATH_M2) != 0)
            {
            ...
            if((GET_TMDS_RX0_FORCE_BCKGRD_PATH() & _DISPLAY_PATH_S1) != 0)
            {
            ...
            if((GET_TMDS_RX0_FORCE_BCKGRD_PATH() & _DISPLAY_PATH_S2) != 0)
            {
            ...
            ScalerSetByte_EXINT(P2_39_HDMI_CTRL_39, _P2_3A_PT_21_PORT_PAGE2_HDMI_SCDC_21);
    ...
    if((ScalerGetBit_EXINT(P2_1F_HDMI_CTRL_1F, _BIT3) == _BIT3) && (ScalerGetBit_EXINT(P2_1C_HDMI_CTRL_1C, _BIT7) == _BIT7))
    {
        ...
        ScalerTMDSRx0ReadRequestProc_EXINT0(ScalerGetByte_EXINT(P2_1C_HDMI_CTRL_1C) & 0x70);
    }
    ...
    if(ScalerGetBit_EXINT(P2_23_HDMI_CTRL_23, (_BIT6 | _BIT1)) == (_BIT6 | _BIT1))
    {
        ...
        if(ScalerGetBit_EXINT(P2_23_HDMI_CTRL_23, (_BIT2 | _BIT0)) == 0x00)
        {
            ...
            if((bit)ScalerTMDSRx0CheckCDRStable_EXINT(g_usTMDSRx0ClkCount) == _TRUE)
            {
                // Disable Rising to Falling IRQ, Set IRQ Status Falling to Rising edge IRQ
                ScalerSetBit_EXINT(P2_23_HDMI_CTRL_23, ~(_BIT6 | _BIT2 | _BIT1), (_BIT2 | _BIT1));
...
bit ScalerTMDSRx0CheckCDRStable(WORD usClkCount)
{
    ...
    if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        switch(GET_TMDS_RX0_CLK_MODE())
        {
            ...
            case _TMDS_MHL_PPMODE_PHY_SETTING:
                ...
                if(abs(usClkCount - (ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED) * 4)) > (usClkCount >> 6))
                {
                    DebugMessageRx0("R Channel CDR Unstable", ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED));
                    return _FALSE;
                }
            ...
            case _TMDS_24BIT_PHY_SETTING:
                ...
                // Check Current Measured Freq. is the same as previously locked
                if(abs(usClkCount - (ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED) * 3)) > (usClkCount >> 6))
                {
                    DebugMessageRx0("R Channel CDR Unstable", ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED));
                    return _FALSE;
                }
...
#endif
    {
        ...
        {
            // Check Current Measured Freq. is the same as previously locked
            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }

            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk(_CLK_SOURCE_BLUE) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }

            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk(_CLK_SOURCE_GREEN) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }
        }
        ...
        {
            // Check Current Measured Freq. is the same as previously locked
            if(abs(usClkCount - ScalerTMDSRx0MeasureClk(_CLK_SOURCE_RED)) > (usClkCount >> 6))
            {
                return _FALSE;
            }

            if(abs(usClkCount - ScalerTMDSRx0MeasureClk(_CLK_SOURCE_BLUE)) > (usClkCount >> 6))
            {
                return _FALSE;
            }

            if(abs(usClkCount - ScalerTMDSRx0MeasureClk(_CLK_SOURCE_GREEN)) > (usClkCount >> 6))
            {
                return _FALSE;
            }
        }
...
// Output Value : _TRUE => Stable
//--------------------------------------------------
BYTE ScalerTMDSRx0CheckCDRStable_EXINT0(WORD usClkCount) using 1
{
#if(_D0_INPUT_PORT_TYPE == _D0_MHL_PORT)
    ...
    if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        switch(GET_TMDS_RX0_CLK_MODE())
        {
            ...
            case _TMDS_MHL_PPMODE_PHY_SETTING:
                ...
                // Check Current Measured Freq. is the same as previously locked
                if(abs(usClkCount - ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_TMDS)) > (usClkCount >> 6))
                {
                    return _FALSE;
                }

                // Check Current Measured Freq. is the same as previously locked
                if(abs(usClkCount - (ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_RED) * 4)) > (usClkCount >> 6))
                {
                    return _FALSE;
                }
            ...
            case _TMDS_24BIT_PHY_SETTING:
                ...
                // Check Current Measured Freq. is the same as previously locked
                if(abs(usClkCount - (ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_RED) * 3)) > (usClkCount >> 6))
                {
                    return _FALSE;
                }
...
#endif
    {
        ...
        if(GET_TMDS_RX0_HIGH_SPEED_MODE() == _TRUE)
        {
            ...
            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_RED) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }

            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_BLUE) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }

            if(abs(usClkCount - (ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_GREEN) * 4)) > (usClkCount >> 5))
            {
                return _FALSE;
            }
        }
...
#endif
        {
            ...
            if(abs(usClkCount - ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_RED)) > (usClkCount >> 6))
            {
                return _FALSE;
            }

            if(abs(usClkCount - ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_BLUE)) > (usClkCount >> 6))
            {
                return _FALSE;
            }

            if(abs(usClkCount - ScalerTMDSRx0MeasureClk_EXINT0(_CLK_SOURCE_GREEN)) > (usClkCount >> 6))
            {
                return _FALSE;
            }
        }
...
WORD ScalerTMDSRx0MeasureClk(BYTE ucMeasureClk)
{
    ...
    if(GET_D0_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        ...
        ScalerSetByte(PB_A1_CMU_01, 0x91);
    }
    else
#endif
    {
        // Enable CMU_EN(Lane0,1,2) and Rx_EN(Lane0,1,2,3)
        ScalerSetByte(PB_A1_CMU_01, 0xF7);
    }
    ...
    // Start Measure
    ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_28_PORT_PAGE2_HDMI_NTX1024TR0, ~_BIT3, _BIT3);
...
// Output Value : Measured Clk Count
//--------------------------------------------------
WORD ScalerTMDSRx0MeasureClk_EXINT0(BYTE ucMeasureClk) using 1
{
    BYTE ucDelayCount = 0;
    WORD usClkCount = 0;
    BYTE ucBackUp = 0;
    ...
    {
        // Enable CMU_EN(Lane0) and Rx_EN(Lane0,3)
        ScalerSetByte_EXINT(PB_A1_CMU_01, 0x91);
    }
    else
#endif
    {
        // Enable CMU_EN(Lane0,1,2) and Rx_EN(Lane0,1,2,3)
        ScalerSetByte_EXINT(PB_A1_CMU_01, 0xF7);
    }
    ...
    ScalerSetBit_EXINT(P2_CA_HDMI_DP, ~_BIT3, _BIT3);
...
bit ScalerTMDSRx0HdmiDeepColorProc(WORD usTmdsClkCnt)
{
    ...
    if(ScalerTMDSRx0HdmiSetDeepColorPll(usTmdsClkCnt, GET_COLOR_DEPTH(_D0_INPUT_PORT)) == _FALSE)
    {
        return _FALSE;
    }

    // Wait for Phase lock
    if(ScalerTMDSRx0HdmiWaitDeepColorPllLock() == _FALSE)
    {
        return _FALSE;
    }
...
bit ScalerTMDSRx0HdmiSetDeepColorPll(WORD usTMDSClkCount, BYTE ucColorDepth)
{
...
#if((_D0_HDMI_2_0_SUPPORT == _ON) && (_HDMI_2_0_YCBCR420_SUPPORT == _ON))
    BYTE ucRatio = 1;
#endif
    ...
    ScalerSetBit(PB5_A7_MN_SCLKG_TRK_CTRL, ~(_BIT7 | _BIT6 | _BIT1 | _BIT0), (_BIT1 | _BIT0));
    ...
    ScalerSetBit(PB5_A7_MN_SCLKG_TRK_CTRL, ~(_BIT1 | _BIT0), 0x00);
    ...
    ScalerSetBit(PB5_A0_MN_SCLKG_CTRL, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT4));
...
void ScalerTMDSRx0HdmiPacketDetect(bit bWait)
{
    if(bWait == _WAIT)
    {
        ...
        CLR_HDMI_RX0_PACKET_DETECTING();
    }
    else
    {
        if(ScalerGetBit(P2_CC_HDMI_GPVS, _BIT6) == _BIT6)
        {
            ScalerTMDSRx0HdmiWatchDog(_HDMI_AV_MUTE_WD, _ENABLE);

            SET_HDMI_RX0_GCP_RECEIVED();
        }
        else
        {
            ScalerTMDSRx0HdmiWatchDog(_HDMI_AV_MUTE_WD, _DISABLE);
        ...
        if(ScalerGetBit(P2_CC_HDMI_GPVS, _BIT5) == _BIT5)
        {
            ...
            ScalerTMDSRx0HdmiWatchDog(_HDMI_AVI_INFO_VARIATION_WD, _ENABLE);
        ...
        else
        {
            ...
            ScalerTMDSRx0HdmiWatchDog(_HDMI_AVI_INFO_VARIATION_WD, _DISABLE);
        ...
        SET_HDMI_RX0_PACKET_DETECTING();

        // Activate Packet Detection Event
        ScalerTimerActiveTimerEvent(SEC(0.13), _SCALER_TIMER_EVENT_RX0_TMDS_HDMI_PACKET_DETECT);
    }
}
...
BYTE ScalerTMDSRx0HdmiGetColorDepth(void)
{
    if(GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR422)
    {
        return _COLOR_DEPTH_12_BITS;
    }

    if(GET_HDMI_RX0_GCP_RECEIVED() == _TRUE)
    {
    ...
    if(bEnable == _ENABLE)
    {
        if((ucWDType & _HDMI_AVI_INFO_VARIATION_WD) == _HDMI_AVI_INFO_VARIATION_WD)
        {
            if(ScalerGetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_41_PORT_PAGE2_HDMI_VWDSR, _BIT0) == 0x00)
            {
                ...
                if((GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR444) || (GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR422) || (GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR420))
                {
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0PSIntHandler_EXINT0(void) using 1
{
    BYTE pucBackUp[2];

    pucBackUp[0] = ScalerGetByte_EXINT(P2_39_HDMI_CTRL_39);
    pucBackUp[1] = ScalerGetByte_EXINT(P2_38_HDMI_CTRL_38);
    ...
    ScalerSetByte_EXINT(P2_38_HDMI_CTRL_38, pucBackUp[1]);
    ScalerSetByte_EXINT(P2_39_HDMI_CTRL_39, pucBackUp[0]);
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0ReadRequestProc_EXINT0(BYTE ucRROutCome) using 1
{
    ...
    switch(ucRROutCome)
    {
        ...
        case _HDMI_2_0_READ_REQUEST_NACK2:
            ...
            if(GET_HDMI_2_0_RX0_RR_ENABLE() == _TRUE)
            {
                ...
                ScalerTimerWDActivateTimerEvent_EXINT0(10, _SCALER_WD_TIMER_EVENT_RX0_HDMI_RR_RETRY);
            }
#endif
        ...
        if((pData[0] == 0x1A) && (pData[1] == 0x00) && (pData[2] == 0x00))
        {
            switch(enumContent)
            {
                case _SPD_INFO_FREESYNC_SUPPORT:
                    ...
                    return ((bit)ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x95, _BIT0));

                case _SPD_INFO_FREESYNC_ENABLE:

                    return ((bit)ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x95, _BIT1));

                case _SPD_INFO_FREESYNC_ACTIVE:

                    return ((bit)ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x95, _BIT2));
...
void ScalerTMDSRx0TMDSScanInitial(BYTE ucInputPort)
{
    ...
    ScalerTMDSRx0Initial(ucInputPort);
...
bit ScalerTMDSRx0TMDSPSPreDetect(BYTE ucInputPort)
{
    ...
    if(GET_TMDS_RX0_DDC_DETECTED() == _TRUE)
    {
        DebugMessageRx0("7. PowerSaving D0 DDC", 0);
...
bit ScalerTMDSRx0TMDSNormalPreDetect(BYTE ucInputPort)
{
    ...
    if(GET_TMDS_RX0_DDC_DETECTED() == _TRUE)
    {
        DebugMessageRx0("7. Normal D0 DDC", 0);
...
BYTE ScalerTMDSRx0TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX0_PHY_SET() == _TRUE)
    {
...
#if(_D0_HDMI_2_0_SUPPORT == _ON)
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) ||
           ((GET_D0_TMDS_MODE() == _TMDS_MHL_MODE) && (ScalerMHLRx0DetectClkMode() == _FALSE)) ||
           (ScalerTMDSRx0TMDSVideoDetect(_WAIT) == _TMDS_VIDEO_DATA_NONE))
#else
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) ||
           ((GET_D0_TMDS_MODE() == _TMDS_MHL_MODE) && (ScalerMHLRx0DetectClkMode() == _FALSE)))
#endif
        ...
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerTMDSRx0TMDSVideoDetect(_WAIT) == _TMDS_VIDEO_DATA_NONE))
#else
        if(ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE)
#endif
...
#endif // End of #if(_D0_INPUT_PORT_TYPE == _D0_MHL_PORT)
        {
            ...
            if((GET_TMDS_RX0_HIGH_SPEED_MODE() == _FALSE) && (GET_D0_TMDS_MODE() != _TMDS_MHL_MODE)) // HDMI mode
            {
                DebugMessageRx0("TMDS Rx0 First 1:10 set phy not success", 1);
        ...
        DebugMessageRx0("9. Freq Check Pass => Pixel Clock", GET_D0_INPUT_PIXEL_CLK());
        DebugMessageRx0("7. TMDS Lane0 LE Min", g_pucTMDSRx0LEMin[0]);
        DebugMessageRx0("7. TMDS Lane1 LE Min", g_pucTMDSRx0LEMin[1]);
        DebugMessageRx0("7. TMDS Lane2 LE Min", g_pucTMDSRx0LEMin[2]);
        DebugMessageRx0("7. TMDS Lane0 LE Max", g_pucTMDSRx0LEMax[0]);
        DebugMessageRx0("7. TMDS Lane1 LE Max", g_pucTMDSRx0LEMax[1]);
        DebugMessageRx0("7. TMDS Lane2 LE Max", g_pucTMDSRx0LEMax[2]);
        ...
        ScalerSetBit(P2_0B_HDMI_CTRL_0B, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2));

        // Enable Character Error Detction Function Lock Transition IRQ
        ScalerSetBit(P2_18_HDMI_CTRL_18, ~(_BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1 | _BIT0));

        // Set HDMI 2.0 Character Error Detection Function Lock Transition Error Boundary
        ScalerSetBit(P2_14_HDMI_CTRL_14, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (GET_D0_INPUT_PIXEL_CLK() / 10) >> 8);
        ScalerSetByte(P2_15_HDMI_CTRL_15, (GET_D0_INPUT_PIXEL_CLK() / 10));

        DebugMessageRx0("HDMI 2.0 Error Boundary set High Byte", ScalerGetByte(P2_14_HDMI_CTRL_14));
        DebugMessageRx0("HDMI 2.0 Error Boundary set Low Byte", ScalerGetByte(P2_15_HDMI_CTRL_15));
#endif

        if(ScalerTMDSRx0FormatDetect() == _SOURCE_HDMI)
        {
            DebugMessageRx0("Format Detect HDMI", 0x00);
        ...
        if(((GET_MHL_RX0_ADOPTER_ID_HBYTE() == _ASUS_PADFONE2_ADOPTER_ID_HBYTE) && (GET_MHL_RX0_ADOPTER_ID_LBYTE() == _ASUS_PADFONE2_ADOPTER_ID_LBYTE)) &&
           !(ScalerGetBit(P2_A9_TMDS_ABC0, (_BIT7 | _BIT6)) == (_BIT7 | _BIT6)))
        {
            return _SOURCE_NONE;
        }
#endif

        ucTMDSVideoDataType = ScalerTMDSRx0TMDSVideoDetect(_WAIT);

        if(ucTMDSVideoDataType == _TMDS_VIDEO_DATA_DE_ONLY)
        {
            DebugMessageRx0("DE Only Mode", 0x00);
        ...
        if(GET_D0_INPUT_PIXEL_CLK() > 3400)
        {
            // [1] De_inverse = 1'b1 --> Adapation Only for Data Area (Transition Minimum)
            // [0] De_brank_en = 1'b1 --> Enable Blank Function of gdi_de
            ScalerSetBit(P1D_AF_L0_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
            ScalerSetBit(P1D_BF_L1_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
            ScalerSetBit(P1D_CF_L2_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
        }
        else
        {
            // [1] De_inverse = 1'b1 --> Adapation Only for Data Area (Transition Minimum)
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            ScalerTMDSRx0HdmiPacketDetect(_WAIT);

            ScalerSyncTMDSRx0HdmiVideoSetting();

            SET_COLOR_DEPTH(_D0_INPUT_PORT, ScalerTMDSRx0HdmiGetColorDepth());

            if((GET_COLOR_DEPTH(_D0_INPUT_PORT) != _COLOR_DEPTH_8_BITS) && (GET_COLOR_SPACE(_D0_INPUT_PORT) != _COLOR_SPACE_YCBCR422))
            {
                DebugMessageRx0("Deep Color Mode Detected", GET_COLOR_DEPTH(_D0_INPUT_PORT));
                SET_D0_INPUT_PIXEL_CLK((GET_D0_INPUT_PIXEL_CLK() << 3) / GET_COLOR_DEPTH(_D0_INPUT_PORT));

                if(ScalerTMDSRx0HdmiDeepColorProc(g_usTMDSRx0ClkCount) == _FALSE)
                {
                    return _SOURCE_NONE;
                }
                ...
                if(GET_COLOR_SPACE(_D0_INPUT_PORT) == _COLOR_SPACE_YCBCR420)
                {
                    ...
                    ScalerSetBit(P2_AD_TMDS_CPS, ~_BIT7, _BIT7);
                }
                else
                {
                    // HDMI Output Div2 Disable
                    ScalerSetBit(P2_AD_TMDS_CPS, ~_BIT7, 0x00);
                }
            }
            else
            {
...
#if(_HDMI_FREESYNC_SUPPORT == _ON)
            if(ScalerTMDSRx0HdmiGetAmdSpdInfo(_SPD_INFO_FREESYNC_SUPPORT) == _ENABLE)
            {
                SET_HDMI_FREESYNC_ENABLED();

                // Get SPD Free Sync Vfreq Max
                g_usFREESYNCVFreq = (WORD)ScalerTMDSRx0HdmiGetAmdSpdInfo(_SPD_INFO_FREESYNC_MAX_VFREQ) * 10;
    ...
    if(ScalerTimerPollingFlagProc(130, P2_CB_HDMI_SR, _BIT0, _TRUE) == _TRUE)
    {
        ...
        if(GET_D0_TMDS_MODE() == _TMDS_HDMI_MODE)
        {
            ...
            if(ScalerGetBit(P2_01_HDMI_CTRL_01, _BIT3) == _BIT3)
            {
                ...
                ScalerSetDataPortBit(P2_39_HDMI_CTRL_39, _P2_3A_PT_21_PORT_PAGE2_HDMI_SCDC_21, ~_BIT0, _BIT0);
            }
        }
#endif
...
BYTE ScalerTMDSRx0TMDSVideoDetect(bit bWait)
{
    ...
    else
    {
        ...
        SET_TMDS_RX0_RGBHV_DETECTING();

        // Activate TMDS video Detection Event
        ScalerTimerActiveTimerEvent(SEC(0.045), _SCALER_TIMER_EVENT_RX0_TMDS_VIDEO_DETECT);
...
bit ScalerTMDSRx0TMDSStableDetect(BYTE ucSourceType)
{
    ...
    if((GET_D0_TMDS_MODE() == _TMDS_HDMI_MODE) && (ScalerGetBit(P2_00_HDMI_CTRL_00, _BIT0) == _BIT0))
    {
    ...
    if(GET_TMDS_RX0_Z0_ASSERTED() == _TRUE)
    {
        if((ScalerGetBit(P2_A9_TMDS_ABC0, _BIT6) == _BIT6) && (ScalerGetBit(P2_A9_TMDS_ABC0, (_BIT7 | _BIT5)) != (_BIT7 | _BIT5)))
        {
            SET_TMDS_RX0_HPD_TRIGGER_EVENT(_TMDS_HPD_NORMAL);

            CLR_TMDS_RX0_Z0_ASSERTED();
        ...
        else if((ScalerGetBit(P2_A9_TMDS_ABC0, (_BIT7 | _BIT5)) == (_BIT7 | _BIT5)))
        {
            CLR_TMDS_RX0_Z0_ASSERTED();
    ...
    {
        if(GET_TMDS_RX0_RGBHV_DETECTING() == _FALSE)
        {
            // TMDS Video Detected
            if(ScalerTMDSRx0TMDSVideoDetect(_NOT_WAIT) != _TMDS_VIDEO_DATA_RGBHV)
            {
    ...
    if(ucSourceType == _SOURCE_HDMI)
    {
        if(GET_HDMI_RX0_PACKET_DETECTING() == _FALSE)
        {
            // HDMI Packet Detected
            ScalerTMDSRx0HdmiPacketDetect(_NOT_WAIT);
            ...
            if(GET_HDMI_RX0_AVI_INFO_FRAME_RECEIVED() == _TRUE)
            {
                ...
                if(ScalerTMDSRx0HdmiAviPixelRepInfoChanged() == _TRUE)
                {
            ...
            else
            {
                ...
                if(GET_COLOR_RGB_QUANTIZATION_RANGE(_D0_INPUT_PORT) != _RGB_QUANTIZATION_RESERVED)
                {
            ...
            if(GET_HDMI_RX0_GCP_RECEIVED() == _TRUE)
            {
                if((GET_COLOR_SPACE(_D0_INPUT_PORT) != _COLOR_SPACE_YCBCR422) && (ScalerTMDSRx0HdmiGetColorDepth() != GET_COLOR_DEPTH(_D0_INPUT_PORT)))
                {
                    DebugMessageRx0("Color Depth Has Changed", GET_COLOR_DEPTH(_D0_INPUT_PORT));
                    return _FALSE;
                }
            }
            else
            {
                if((GET_COLOR_SPACE(_D0_INPUT_PORT) != _COLOR_SPACE_YCBCR422) && (GET_COLOR_DEPTH(_D0_INPUT_PORT) != _COLOR_DEPTH_8_BITS))
                {
            ...
            if(GET_TMDS_RX0_SPD_INFO_FRAME_RECEIVED() == _TRUE)
            {
                if((bit)ScalerTMDSRx0HdmiGetAmdSpdInfo(_SPD_INFO_FREESYNC_SUPPORT) != GET_HDMI_FREESYNC_ENABLED())
                {
...
#if(_HDMI_FREESYNC_SUPPORT == _ON)
        if(GET_TMDS_RX0_FREESYNC_SUPPORT() != GET_FREESYNC_SUPPORT())
        {
            SET_TMDS_RX0_FREESYNC_SUPPORT(GET_FREESYNC_SUPPORT());
            SET_TMDS_RX0_HPD_TRIGGER_EVENT(_TMDS_HPD_NORMAL);
...
void ScalerTMDSRx0HotPlugEvent(void)
{
    ...
    {
        // Save Z0 Status
        ucZ0Status = ScalerGetBit(PB_A2_CMU_02, (_BIT7 | _BIT6 | _BIT5 | _BIT4));
        ...
        // Restore Z0 Status
        ScalerSetBit(PB_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), ucZ0Status);
