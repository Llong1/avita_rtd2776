...
void ScalerDpRx0Tp1SetPhy_EXINT0(EnumDpNFCodeRef enumDpNFCodeRef) using 1
{
    ...
    ScalerSetBit_EXINT(P1D_22_CMU_08, ~(_BIT3 | _BIT2 | _BIT1), _BIT3);
    ...
    if(ScalerGetBit_EXINT(PB7_DC_AUX_IRQ_STATUS, _BIT7) == _BIT7)
    {
        ...
        if(ScalerGetBit_EXINT(PB7_FB_DUMMY_1, (_BIT7 | _BIT6)) == (_BIT7 | _BIT6))
        {
            ...
            if(ucDPCDAddress <= 0x0F)
            {
                ...
                {
                    ScalerSetByte_EXINT(PB7_D9_AUX_TX_DATA, _D0_DP_LINK_CLK_RATE);
                    ucDPCDAddress++;
                    pData_EXINT[1]--;
                }
                ...
                else
                {
                    ...
                    {
                        ScalerSetByte_EXINT(PB7_D9_AUX_TX_DATA, ScalerDpRx0GetDpcdInfo_EXINT0(0x00, 0x00, (ucDPCDAddress + pData_EXINT[2])));
                    }
                }
            }
            else
        ...
        if(ScalerGetBit_EXINT(PB7_DA_AUX_FIFO_RST, (_BIT2 | _BIT1)) == (_BIT2 | _BIT1)) // Write 68xxxh into INT0
        {
            ...
            if((ScalerGetBit_EXINT(PB_1A_HDCP_IRQ, _BIT1) == _BIT1) && (ScalerGetBit_EXINT(PB_20_HDCP_DEBUG, _BIT7) == _BIT7))
            {
