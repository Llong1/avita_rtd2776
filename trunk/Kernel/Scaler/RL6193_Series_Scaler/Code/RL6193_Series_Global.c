...
void ScalerGlobalPathWatchDog(EnumInputDataPath enumInputDataPath, bit bEnable)
{
    ...
    {
        // Enable S2 WD to prevent WD trigger overlay disable
        if((bEnable == _ENABLE) &&
        ...
        {
            ScalerSetBit(PC0_0C_S2_WATCH_DOG_CTRL0, ~_BIT1, _BIT1);
        }
    }
    else
    {
...
void ScalerGlobalIDDomainDBApply(EnumInputDataPath enumSelectedDataPath, EnumDBApply enumDBApply)
{
    ...
    if(enumDBApply == _DB_APPLY_POLLING)
    {
        ...
        else
        {
            ...
            {
                usSelectedPage = _REG_PAGE_OFFSET * ucDataPathIndex;
                if(ScalerGetBit(P5_AE_DISP_M1_DISP_CTRL2 + usSelectedPage, _BIT2) != 0x00)
                {
                    ScalerSetBit(P5_AE_DISP_M1_DISP_CTRL2 + usSelectedPage, ~_BIT2, 0x00);
                    ScalerSetBit(P5_AD_DISP_STATUS + usSelectedPage, ~_BIT4, _BIT4);
                }
            }
        }
    }
