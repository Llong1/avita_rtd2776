...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_Measure.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
bit ScalerMeasureCheckOfflineMeasureChange(EnumSourceSearchPort enumSourceSearchPort, BYTE ucResultIndex);
...
void ScalerMeasureUpdateOnlineMeasureResult(EnumInputDataPath enumInputDataPath);
...
bit ScalerMeasureTimingChange(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort, BYTE ucResultIndex)
{
    // Check on-line & off-line measure results
    bit bTimingChangeStatus = ((ScalerMeasureCheckOnlineMeasureChange(enumSourceSearchPort, ucResultIndex) == _TRUE) ||
                               (ScalerMeasureCheckOfflineMeasureChange(enumSourceSearchPort, ucResultIndex) == _TRUE) ||
    ...
    if(GET_PAGED_BIT(ucOffset, P16_02_MEAS_CTL1, _BIT7) == 0x00)
    {
...
#endif
        {
...
#endif
            {
                ...
                // Check on-line measure H total after display setting
                if(g_pstRegionTimingInfo[ucResultIndex].stMeasureSyncData.usHTotal != 0xFFFF)
                {
                    bTimingChangeStatus |= (abs(stMeasureSyncHV.usHTotal - g_pstRegionTimingInfo[ucResultIndex].stMeasureSyncData.usHTotal) > 2);
                }
            }
        }
    }
...
//--------------------------------------------------
// Description  : Off-line measure Dempsey Roll, checking every available port
// Input Value  : enumSourceSearchPort --> check port
//                ucResultIndex --> result index
// Output Value : _TRUE or _FALSE
//--------------------------------------------------
bit ScalerMeasureCheckOfflineMeasureChange(EnumSourceSearchPort enumSourceSearchPort, BYTE ucResultIndex)
{
    ...
    // Check the validity of off-line measure target
    if(g_pstRegionTimingInfo[g_ucOfflineMeasureTarget].stTimingInfo.enumInputDataPath == 0x00)
    {
        g_ucOfflineMeasureTarget = ucResultIndex;
    }

    // Check only for indexed port
    if(g_ucOfflineMeasureTarget == ucResultIndex)
    {
#if(_VGA_SUPPORT == _ON)
        if(g_pstRegionTimingInfo[g_ucOfflineMeasureTarget].stMeasureSyncData.b4MeasurePort != _A0_INPUT_PORT)
#endif
        {
            // Check if current off-line measure target being modified elsewhere
            if(ScalerMeasureGetOfflineMeasurePort() != g_pstRegionTimingInfo[g_ucOfflineMeasureTarget].stMeasureSyncData.b4MeasurePort)
            {
                // Restart off-line measure
                ScalerMeasureSetOfflineMeasure(g_pstRegionTimingInfo[g_ucOfflineMeasureTarget].stMeasureSyncData.b4MeasurePort);
                return _FALSE;
            }
            else if(ScalerGetBit(P30_02_MEAS_CTL1, _BIT7) == _BIT7)
            {
                // Off-line measure is now measuring
                return _FALSE;
            }
            else
            {
                ...
                else
                {
                    ...
                    // Pop out result & check H width & V height
                    bCheckResult = ((abs(stMeasureSyncHV.usHWidth - g_pstRegionTimingInfo[ucResultIndex].stTimingInfo.usHWidth) > 0x02) ||
                                    (abs(stMeasureSyncHV.usVHeight - g_pstRegionTimingInfo[ucResultIndex].stTimingInfo.usVHeight) > 0x02));
                }
            }
        }

        // Update off-line measure target
        do
        {
            ucResultIndex = (ucResultIndex + 1) % _MULTI_DISPLAY_MAX;

            if(g_pstRegionTimingInfo[ucResultIndex].stTimingInfo.enumInputDataPath != 0x00)
            {
                g_ucOfflineMeasureTarget = ucResultIndex;
                break;
            }
        }while(ucResultIndex != g_ucOfflineMeasureTarget);
    }
...
bit ScalerMeasureFinalSetting(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    // Set measure H total to 0xFFFF to prevent timing check
    g_pstRegionTimingInfoPointer->stMeasureSyncData.usHTotal = 0xFFFF;
    ...
    switch(enumSourceSearchPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            if(ScalerColor420To422D0Recheck(GET_MEASURE_INPUT_PATH(), (BYTE)(g_pstRegionTimingInfoPointer - g_pstRegionTimingInfo)) == _FALSE)
            {
                DebugMessageMeasure("D0 420 re-check fail", 0x00);
                return _FALSE;
            }
#endif
#endif
        ...
        case _D1_INPUT_PORT:
            ...
            if(ScalerColor420To422D1Recheck(GET_MEASURE_INPUT_PATH(), (BYTE)(g_pstRegionTimingInfoPointer - g_pstRegionTimingInfo)) == _FALSE)
            {
                DebugMessageMeasure("D1 420 re-check fail", 0x00);
                return _FALSE;
            }
#endif
#endif
...
bit ScalerMeasureGetAutoResult(StructAutoActiveData *pstActiveRegion, EnumInputDataPath enumInputDataPath, bit bWait)
{
    ...
    pstActiveRegion->usVActiveStart = (((WORD)GET_PAGED_BIT(ucOffset, P0_7E_VER_START_END_H, 0xF0) << 4) |
                                       ((WORD)GET_PAGED_BYTE(ucOffset, P0_7F_VER_START_L)));

    pstActiveRegion->usVActiveEnd = (((WORD)GET_PAGED_BIT(ucOffset, P0_7E_VER_START_END_H, 0x0F) << 8) |
    ...
    switch(enumSourceType)
    {
        ...
        case _SOURCE_DVI:
            ...
            switch(enumSourceSearchPort)
            {
                ...
                case _D1_INPUT_PORT:
                    usHdmiPage = 0x7000 - 0x0200;
                    break;
#endif

#if(_D2_TMDS_SUPPORT == _ON)
                case _D2_INPUT_PORT:
                    usHdmiPage = 0x7100 - 0x0200;
                    break;
#endif

#if(_D3_TMDS_SUPPORT == _ON)
                case _D3_INPUT_PORT:
                    usHdmiPage = 0x7200 - 0x0200;
                    break;
#endif

#if(_D4_TMDS_SUPPORT == _ON)
                case _D4_INPUT_PORT:
                    usHdmiPage = 0x7300 - 0x0200;
                    break;
#endif

#if(_D5_TMDS_SUPPORT == _ON)
                case _D5_INPUT_PORT:
                    usHdmiPage = 0x7400 - 0x0200;
                    break;
#endif
            }

            if(ScalerGetBit((usHdmiPage + P2_A7_TMDS_PWDCTL), _BIT7) == _BIT7)
            {
                if(ScalerGetBit((usHdmiPage + P2_A4_TMDS_CTRL), (_BIT4 | _BIT3)) == (_BIT4 | _BIT3))
                {
                    // Read HS/VS polarity from MAC
                    g_pstRegionTimingInfoPointer->stTimingInfo.b1HSP = (ScalerGetBit((usHdmiPage + P2_C0_HDCP_CR), _BIT6) == 0x00);
                    g_pstRegionTimingInfoPointer->stTimingInfo.b1VSP = (ScalerGetBit((usHdmiPage + P2_C0_HDCP_CR), _BIT5) == 0x00);
                }
                else
                {
            ...
            {
                // Set HDMI VS intert
                ScalerSetBit((usHdmiPage + P2_AF_TMDS_WDC), ~_BIT3, _BIT3);
            ...
            {
                // Set HDMI HS intert
                ScalerSetBit((usHdmiPage + P2_AF_TMDS_WDC), ~_BIT2, _BIT2);
...
bit ScalerMeasureGetOfflineMeasureResult(StructMeasureSyncData *pstMeasureSyncHV)
{
...
#endif
    {
        ...
        // offline measure 2 line measure enable
        if(ScalerGetBit(P30_1C_MEASURE_DUMMY0, _BIT0) == _BIT0)
        {
            pstMeasureSyncHV->usVSPeriod = (pstMeasureSyncHV->usVSPeriod << 1);
        }
    }
    ...
    if(ScalerGetBit(P30_1C_MEASURE_DUMMY0, _BIT0) == _BIT0)
    {
        pstMeasureSyncHV->usVHeight = (pstMeasureSyncHV->usVHeight << 1);
    }
    ...
    if(ScalerGetBit(P30_00_SYNC_SELECT, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00)
    {
    ...
    else if(ScalerGetBit(P30_00_SYNC_SELECT, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == _BIT0)
    {
        if(ScalerGetBit(P30_00_SYNC_SELECT, _BIT6) == _BIT6)
        {
            pstMeasureSyncHV->usHTotal = (pstMeasureSyncHV->usHTotal << 1);
            pstMeasureSyncHV->usHWidth = (pstMeasureSyncHV->usHWidth << 1);
        }
...
bit ScalerMeasureGetOnlineMeasureResult(StructMeasureSyncData *pstMeasureSyncHV, EnumInputDataPath enumInputDataPath)
{
    ...
    // VSync Period
    pstMeasureSyncHV->usVSPeriod = ((((WORD)GET_PAGED_BYTE(ucOffset, P16_08_MEAS_VS_PERIOD_A_M) & 0x0F) << 8) | (GET_PAGED_BYTE(ucOffset, P16_09_MEAS_VS_PERIOD_A_L))) + 1;
    ...
    // VSync Pulse Width
    pstMeasureSyncHV->usVSWidth = ((((WORD)GET_PAGED_BYTE(ucOffset, P16_0C_MEAS_VS_PULSE_A_M) & 0xF) << 8) | (GET_PAGED_BYTE(ucOffset, P16_0D_MEAS_VS_PULSE_A_L))) + 1;
    ...
    // V Height
    pstMeasureSyncHV->usVHeight = ((((WORD)GET_PAGED_BYTE(ucOffset, P16_11_MEAS_VS_PERIOD_D_M) & 0x0F) << 8) | (GET_PAGED_BYTE(ucOffset, P16_12_MEAS_VS_PERIOD_D_L))) + 1;
...
//--------------------------------------------------
// Description  : Update on-line measure result for timing change check
...
void ScalerMeasureUpdateOnlineMeasureResult(EnumInputDataPath enumInputDataPath)
{
    ...
    {
        if(g_pstRegionTimingInfo[ucResultIndex].stTimingInfo.enumInputDataPath == enumInputDataPath)
        {
            g_pstRegionTimingInfo[ucResultIndex].stMeasureSyncData.usHTotal = g_pstRegionTimingInfo[ucResultIndex].stTimingInfo.usHTotal;

            // Check 1/2 SD case
            if(ScalerFmtCnvHalfSDGetStatus(enumInputDataPath) == _TRUE)
            {
                g_pstRegionTimingInfo[ucResultIndex].stMeasureSyncData.usHTotal = (g_pstRegionTimingInfo[ucResultIndex].stMeasureSyncData.usHTotal >> 1);
            }

            // Re-start on-line measure
            ScalerMeasureSetOnlineMeasure(enumInputDataPath);
...
void ScalerMeasureSetAuto(EnumMeasureFieldSelect enumFieldSelect, EnumInputDataPath enumInputDataPath)
{
    ...
    // Set V Boundary
    ScalerMeasureAutoFuncVBoundInitial(0x0000, 0x0FFF, enumInputDataPath);
...
void ScalerMeasureSetDisplayRegionCount(EnumDisplayDataPath enumDisplayDataPath, EnumActiveRegionType enumRegion)
{
    switch(enumDisplayDataPath)
    {
        ...
        case _DISPLAY_PATH_S2:
            ScalerSetBit(P30_45_D_ACT_MEAS_CTRL, ~(_BIT7 | _BIT6 | _BIT5), _BIT5);
            break;
...
void ScalerMeasureSetOfflineMeasure(EnumSourceSearchPort enumSourcePort)
{
    ...
    ScalerSetBit(P30_00_SYNC_SELECT, ~(_BIT7 | _BIT6), 0x00);
    ...
    else
    {
        switch(enumSourcePort)
        {
            ...
            case _D1_INPUT_PORT:
                ...
                {
                    ScalerSetBit(P30_00_SYNC_SELECT, ~_BIT6, _BIT6);
                }
...
#if(_FREESYNC_SUPPORT == _ON)
    if((ScalerGlobalGetIDCode() >= _RL6193_VER_E_ID_CODE) && (GET_FREESYNC_ENABLED() == _TRUE))
    {
        // Enable offline measure 2 line measure
        ScalerSetBit(P30_1C_MEASURE_DUMMY0, ~_BIT0, _BIT0);
    }
    else
#endif
    {
        ScalerSetBit(P30_1C_MEASURE_DUMMY0, ~_BIT0, 0x00);
    }
...
void ScalerMeasureInterfaceInitial(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort *penumSourceSearchPort)
{
    ...
    switch(*penumSourceSearchPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            ScalerFmtCnv420To422D0InitialSetting();
#endif
...
#if(_D1_HDMI_2_0_SUPPORT == _ON)
            // Initial D0 420 setting
            ScalerFmtCnv420To422D1InitialSetting();
#endif
...
#if(_D3_INPUT_PORT_TYPE == _D3_DUAL_DVI_PORT)
            // Modify input port select for dual-DVI cases
            *penumSourceSearchPort = _D4_INPUT_PORT;
...
bit ScalerMeasureInterlaceCheck(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    switch(enumSourceType)
    {
        ...
        case _SOURCE_HDMI:
            ...
            ScalerSetDataPortBit((usHdmiPage + P2_C9_HDMI_AP), _P2_CA_PT_50_PORT_PAGE2_HDMI_VCR, ~_BIT6, _BIT6);
...
#endif
            {
                ...
                if(ScalerGetDataPortBit((usHdmiPage + P2_C9_HDMI_AP), _P2_CA_PT_50_PORT_PAGE2_HDMI_VCR, _BIT6) == _BIT6)
                {
                    if(ScalerGetDataPortBit((usHdmiPage + P2_C9_HDMI_AP), _P2_CA_PT_50_PORT_PAGE2_HDMI_VCR, _BIT5) == 0x00)
                    {
            ...
            ScalerSetDataPortBit((usHdmiPage + P2_C9_HDMI_AP), _P2_CA_PT_50_PORT_PAGE2_HDMI_VCR, ~(_BIT6 | _BIT5), (_BIT6 | _BIT5));
...
bit ScalerMeasureDoubleCheck(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort, EnumRegionIndex enumRegionIndex)
{
    ...
    ScalerMeasureUpdateOnlineMeasureResult(g_pstRegionTimingInfo[enumRegionIndex].stTimingInfo.enumInputDataPath);
    ...
    switch(enumSourceType)
    {
        ...
        case _SOURCE_HDMI:
            ...
            switch(enumSourceSearchPort)
            {
                ...
                case _D1_INPUT_PORT:
                    // Clear Interlace toggle and error flag
                    ScalerSetDataPortBit(P70_C9_HDMI_AP, _P70_CA_PT_50_PORT_PAGE70_HDMI_VCR, ~(_BIT6 | _BIT5), (_BIT6 | _BIT5));
                    break;
#endif
...
void ScalerMeasureCheckFirstLineShift(EnumSourceSearchPort enumSourceSearchPort, WORD usFrameHwidth)
{
    ...
    if((ScalerGetBit((usHdmiPage + P2_A7_TMDS_PWDCTL), _BIT7) == _BIT7) &&
       (ScalerGetBit((usHdmiPage + P2_A4_TMDS_CTRL), (_BIT4 | _BIT3)) == (_BIT4 | _BIT3)))
    {
        // Stop auto-function
        ...
        if(abs(PDATA_WORD(2) - usFrameHwidth) <= 1)
        {
            ...
            ScalerSetBit((usHdmiPage + P2_A7_TMDS_PWDCTL), ~_BIT7, 0x00);
        }
    }
}
    ...
    switch(enumSourceSearchPort)
    {
        ...
        case _D3_INPUT_PORT:
            ...
            if(ScalerSyncDualDVIDoubleCheck(enumSourceSearchPort, GET_INPUT_TIMING_HWIDTH(), GET_INPUT_TIMING_VHEIGHT()) == _FALSE)
            {
                ...
                ScalerTMDSRx2DualDVISwitch(!ScalerTMDSRx2GetDualDVIMode());
                return _FALSE;
            }
...
#if(_D3_INPUT_PORT_TYPE == _D3_DUAL_DVI_PORT)
        // Check D4 Port Because Measure use D4 Port
        case _D4_INPUT_PORT:
            ...
            {
                // Current Timing is not Reasonable, Toggle dual DVI format
                ScalerTMDSRx3DualDVISwitch(!ScalerTMDSRx3GetDualDVIMode());
                return _FALSE;
            }
...
bit ScalerMeasureDualDviOnlineCheck(BYTE ucResultIndex, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    if(ScalerGetBit(P31_03_FC_DUAL_DVI_CTRL1, _BIT7) == 0x00)
    {
        return _TRUE;
    }
