...
void ScalerMDomainSetDisplayFormat(BYTE ucMemorySel)
{
    ...
    if((GET_MDOMAIN_DISPLAY_MODE() == _DISPLAY_MODE_1P) || (ucMemorySel == _FREE_RUN_NO_INPUT))
    {
...
#endif
        {
            ...
            ScalerSetByte(P0_2B_DISPLAY_FORMAT_DATA_PORT, (HIBYTE(usWatchDogDvt) & 0x1F));
            ...
            ScalerSetByte(P0_2B_DISPLAY_FORMAT_DATA_PORT, (HIBYTE(usFreerunDvt) & 0x1F));
...
WORD ScalerMDomainGetFRCMemoryBW(void)
{
    ...
    {
        // FRC write BW x 2
        PDATA_DWORD(1) = (((DWORD)usIVtotal * PDATA_WORD(0) * GET_MEMORY_DATA_BIT() / _MEMORY_BUS_WIDTH / 10) * g_stMDomainInputData.usVFreq / 100) * 2;
    }
    else
    {
        // FRC write BW
