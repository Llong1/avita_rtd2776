...
void UserCommonDdcciHandler(void)
{
    ...
    if(GET_RUN_DDCCI_AUTO_CHANNEL_COMMAND() == _FAIL)
    {
        ...
        if(GET_DDCCI_AUTO_SWITCH() == _FALSE &&  GET_OSD_DDCCI_STATUS() == _ON )
        {
...
void UserCommonDdcciInitTx(void)
{
    ...
    g_pucDdcciTxBuf[_DDCCI_LENGTH] = _DDCCI_CONTROL_STATUS_FLAG; // 0x80
    g_pucDdcciTxBuf[_DDCCI_COMMAND] = UserCommonDdcciCalCheckSum(); //CHK
