...
bit ScalerMHLRx2MscHandler(EnumDisplayRegion enumMappedRegion, BYTE ucModeState);
...
void ScalerMHLRx2InitialSettings(void)
{
    ...
    ScalerSetBit(P71_C2_HDCP_PCR, ~_BIT0, _BIT0);

    // Set Encryption Enable / Disable Detect Method to Enc_En First
    ScalerSetDataPortBit(P71_C3_HDCP_AP, _P71_C4_PT_C1_PORT_PAGE71_HDCP_SIR, ~(_BIT7 | _BIT6 | _BIT5 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT3);
#endif
    ScalerMHL3Rx2ECBUSInitialSetting();
#endif
}
...
void ScalerMHLRx2CBUSResetProc(void)
{
    ...
    {
        ScalerMHL3Rx2ECBUSResetProc();

        ScalerMHL3Rx2EnableECBUSMode(_DISABLE);
    }

    // Clear oCBUS -> eCBUS Statue Flags
    CLR_MHL3_RX2_SOURCE_XDEVCAP_SUPP();
    CLR_MHL3_RX2_STATUS_VERSION_3_0();
    CLR_MHL3_RX2_MSC_FEAT_REQ_UNCOMPLETE();

    SET_MHL3_RX2_ECBUS_MODE(_OFF);

    SET_MHL3_RX2_BIST_TESTING(_OFF);
    SET_MHL3_RX2_BIST_TESTING_MODE(_MHL3_ECBUS_BIST_MODE_NULL);
    SET_MHL3_RX2_BIST_ECBUS_DURATION(0x00);
    SET_MHL3_RX2_BIST_IMPEDENCE_MODE(_LOW);
...
void ScalerMHLRx2IntHandler_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(P2A_A4_CBUS_CTRL_04, (_BIT1 | _BIT0)) == (_BIT1 | _BIT0))
    {
        ...
        if((GET_MHL3_RX2_ECBUS_MODE() == _ON) || (GET_MHL3_RX2_BIST_TESTING() == _ON))
        {
            ScalerMHL3Rx2ECBUSResetProc_EXINT0();

            ScalerMHL3Rx2EnableECBUSMode_EXINT0(_DISABLE);
        }
...
bit ScalerMHLRx2MscReadDevCapabilities(EnumMHLMscOffset enumMSCOffset, BYTE *pucData, BYTE ucExtended)
{
    ...
    if(GET_MHL_RX2_READY_TO_TRANSMIT() == _TRUE)
    {
        ...
        {
            ucResultPacket = ScalerMHLRx2MscFIFOSendCommand(_MSC_READ_XDEVCAP, enumMSCOffset, _MSC_NULL_VALUE, 0, pucData);
        }
        else
#endif
        {
...
void ScalerMHLRx2MscCheckDeviceINT(void)
{
    ...
    if((bit)ScalerGetBit(P2A_CE_CBUS_CTRL_2E, _BIT7) == _TRUE)
    {
...
#if(_D2_MHL_VERSION >= _MHL_VERSION_3_0)
        if((GET_MHL3_RX2_SOURCE_XDEVCAP_SUPP() == _TRUE) || (GET_MHL3_RX2_STATUS_VERSION_3_0() == _TRUE))
        {
            if((ScalerMHLRx2MscReadDevCapabilities(_MSC_80_ECBUS_SPEEDS, &pData[0], _MSC_READ_XDEVCAP) == _TRUE) &&
               (ScalerMHLRx2MscReadDevCapabilities(_MSC_81_TMDS_SPEEDS, &pData[1], _MSC_READ_XDEVCAP) == _TRUE) &&
               (ScalerMHLRx2MscReadDevCapabilities(_MSC_82_ECBUS_DEV_ROLES, &pData[2], _MSC_READ_XDEVCAP) == _TRUE) &&
               (ScalerMHLRx2MscReadDevCapabilities(_MSC_83_LOG_DEV_MAPX, &pData[3], _MSC_READ_XDEVCAP) == _TRUE) &&
               (ScalerMHLRx2MscReadDevCapabilities(_MSC_84_ACRFS_DIV_SUPPORT, &pData[4], _MSC_READ_XDEVCAP) == _TRUE))
            {
                if((bit)ScalerGetDataPortBit(P2A_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_DCAP_CHG) == _TRUE)
                {
                    // Write 1 Clear DevCap_Chg Flag
        ...
        /--------------------------------------------------------------- */
        if((bit)ScalerGetDataPortBit(P2A_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_FEAT_REQ) == _TRUE)
        {
            SET_MHL3_RX2_MSC_FEAT_REQ_UNCOMPLETE();
            ...
            if(ScalerMHLRx2MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_REQ_WRT) == _SUCCESS)
            {
                ...
                if((ScalerMHLRx2MscSendWriteBurst(0x40, ucBirstLength, &pucWriteBurstTmp[0], _MHL_WRITE_BURST_WITH_REQ) == _SUCCESS) &&
                   (ScalerMHLRx2MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_FEAT_COMPLETE) == _SUCCESS))
                {
                    ScalerSetDataPortByte(P2A_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_FEAT_REQ);
        ...
        //-----------------------------------------------//
        if(((bit)ScalerGetDataPortBit(P2A_AC_CBUS_CTRL_0C, _MSC_CONNECTED_RDY, _MSC_XDEVCAP_SUPP) == _TRUE) && (GET_MHL3_RX2_SOURCE_XDEVCAP_SUPP() == _FALSE))
        {
            SET_MHL3_RX2_SOURCE_XDEVCAP_SUPP();
...
#endif
            if((GET_MHL_RX2_MHL_VERSION() >= _MHL_VERSION_3_0) && (ScalerGetBit(P2A_00_OCBUS_CTRL_00, _BIT7) == 0x00))
            {
                // Enable MHL 3.0 CBUS Extended MSC Function
                ScalerSetBit(P2A_00_OCBUS_CTRL_00, ~_BIT7, _BIT7);

                // Read XDEVCAP Reg. When Receive XDEVCAP_SUPP, For 3.0 CTS
                if(ScalerMHLRx2MscReadDevCapabilities(_MSC_80_ECBUS_SPEEDS, pData, _MSC_READ_XDEVCAP) == _SUCCESS)
                {
                    // Clear Flag
                    ScalerSetBit(P2A_CE_CBUS_CTRL_2E, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT7);
                }
        ...
        //--------------------------------------------------//
        ScalerGetDataPortByte(P2A_AC_CBUS_CTRL_0C, _MSC_MHL_VERSION_STAT, 1, &pData[0], _AUTOINC);

        if((pData[0] >= 0x30) && (GET_MHL3_RX2_STATUS_VERSION_3_0() == _FALSE) && (ScalerGetBit(P2A_00_OCBUS_CTRL_00, _BIT7) == _BIT7))
        {
            SET_MHL3_RX2_STATUS_VERSION_3_0();
            ...
            if(ScalerMHLRx2MscSendCommand(_MSC_WRITE_XSTAT, _MSC_90_CURR_ECBUS_MODE, _MSC_ECBUS_S_MODE) == _SUCCESS)
            {
                // Clear Flag
                ScalerSetBit(P2A_CE_CBUS_CTRL_2E, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT7);
            }
    ...
    if(((bit)ScalerGetBit(P2A_10_OCBUS_CTRL_10, _BIT1) == _TRUE) && (ScalerGetBit(P2A_00_OCBUS_CTRL_00, _BIT7) == _BIT7))
    {
        //--------------------------------------//
        //---- Get Source MHL AV Link Rate -----//
        //--------------------------------------//
        ScalerGetDataPortByte(P2A_AC_CBUS_CTRL_0C, _MSC_92_AVLINK_MODE_CONTROL, 1, &ucAVLinkNCode, _AUTOINC);

        if(ucAVLinkNCode != GET_MHL3_RX2_AV_LINK_RATE())
        {
            switch(ucAVLinkNCode & (_BIT2 | _BIT1 | _BIT0))
            {
                case _ECBUS_LINK_RATE_1_5:
                    ...
                    ScalerMHLRx2MscSendCommand(_MSC_WRITE_XSTAT, _MSC_91_AVLINK_MODE_STATUS, _MSC_CRU_LOCKED);
            ...
            while((usTmp < 10) && (ScalerMHL3Rx2DataSignalDetect() == _FALSE))
            {
            ...
            if((GET_MHL3_RX2_ECBUS_MODE() == _ON) &&
               (GET_MHL3_RX2_AV_LINK_RATE() != _ECBUS_LINK_RATE_NULL) &&
               (GET_MHL3_RX2_AV_LINK_N_CODE() != _ECBUS_N_CODE_NULL))
            {
#if(_MHL3_ECBUS_AVLINK_DEBUG_MESSAGE == _ON)
                DebugMessageMHL("4-- Rx2 Xstat Written AV Link Rate = ", ucAVLinkNCode);
#endif

#if(_MHL3_ECBUS_AVLINK_CDR_MODE == _DCDR_MODE)
                // ECBUS Data Link Set PHY
                ScalerTMDSRx2MHL3DcdrSetPhy();
                ...
                // Clear Flag
                ScalerSetBit(P2A_10_OCBUS_CTRL_10, ~(_BIT1 | _BIT0), _BIT1);
            }
        }
    }
...
bit ScalerMHLRx2MscHandler(EnumDisplayRegion enumMappedRegion, BYTE ucModeState)
{
    ...
    ScalerMHL3Rx2ECBUSTxTestSettingFunction();
#endif
#if(_ECBUS_RX_TEST_FUNCTION == _ON)
    ScalerMHL3Rx2ECBUSRxTestSettingFunction();
#endif
    ...
    switch(GET_MHL_RX2_PROCESS_STATE())
    {
        ...
        case _MHL_STATE_DISCOVERY_DONE:
            ...
            if((GET_MHL3_RX2_SOURCE_XDEVCAP_SUPP() == _FALSE) && (GET_MHL3_RX2_STATUS_VERSION_3_0() == _FALSE))
            {
                ...
                ScalerTimerActiveTimerEvent(SEC(0.5), _SCALER_TIMER_EVENT_MHL3_RX2_WAIT_SRC_MHL_VERSION);
            }
#endif
    ...
    if((enumMappedRegion != _DISPLAY_RGN_NONE) && (ucModeState != _MODE_STATUS_INITIAL) && (ucModeState != _MODE_STATUS_POWER_OFF))
    {
        switch(GET_MHL_RX2_PROCESS_STATE())
        {
            case _MHL_STATE_SET_DEV_CAP_DONE:
                ...
                if((_D2_MHL_VERSION >= _MHL_VERSION_3_0) &&
                   (GET_MHL3_RX2_WAIT_SRC_MHL_VERSION_TMOUT() == _FALSE) &&
                   (GET_MHL3_RX2_ECBUS_START_UP_TMOUT() == _FALSE))
                {
                    /* ----------------------------------------------------------------------------------------------
                    ...
                    ScalerGetDataPortByte(P2A_AC_CBUS_CTRL_0C, _MSC_DEV_CAT_02, 1, &ucMSCDcapRdy, _AUTOINC);

                    // Ecode CONNECTED_RDY State Reg.
                    ucMSCDcapRdy = ((ucMSCDcapRdy >> 2) & (_MSC_PLIM_STAT | _MSC_POW_STAT)) | _MSC_XDEVCAP_SUPP | _MSC_DCAP_RDY;

                    if((ScalerMHLRx2MscSendCommand(_MSC_WRITE_STAT, _MSC_MHL_VERSION_STAT, _MSC_MHL_VERSION_3_0) == _SUCCESS) &&
                       (ScalerMHLRx2MscSendCommand(_MSC_WRITE_STAT, _MSC_CONNECTED_RDY, ucMSCDcapRdy) == _SUCCESS) &&
                       (ScalerMHLRx2MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_DCAP_CHG) == _SUCCESS))
                    {
                        ...
                        if((GET_MHL3_RX2_ECBUS_MODE() == _OFF) && (GET_MHL3_RX2_BIST_TESTING() == _OFF))
                        {
                            ...
                            ScalerTimerActiveTimerEvent(SEC(1.5), _SCALER_TIMER_EVENT_MHL3_RX2_WAIT_SRC_RAP_CBUS_UP);
                        }

                        DebugMessageMHL("D2 MHL Dev Cap RDY", (0x10 | (GET_MHL3_RX2_ECBUS_MODE() << 1) | (GET_MHL3_RX2_BIST_TESTING())));
...
#endif // End of #if(_D2_MHL_VERSION >= _MHL_VERSION_3_0)
                {
                    ...
                    {
#if(_D2_MHL_VERSION >= _MHL_VERSION_3_0)
                        DebugMessageMHL("D2 MHL Dev Cap RDY", (0x20 | (GET_MHL3_RX2_WAIT_SRC_MHL_VERSION_TMOUT() << 1) | (GET_MHL3_RX2_ECBUS_START_UP_TMOUT())));
#else
            ...
            case _MHL_STATE_DEV_CAP_RDY:
                ...
                {
                    if(((GET_MHL3_RX2_SOURCE_XDEVCAP_SUPP() == _TRUE) || (GET_MHL3_RX2_STATUS_VERSION_3_0() == _TRUE)) &&
                       (GET_MHL3_RX2_ECBUS_MODE() == _ON) && (GET_MHL3_RX2_ECBUS_START_UP_TMOUT() == _FALSE))
                    {
                        if((GET_MHL3_RX2_ECBUS_PROCESS_STATE() > _MHL3_ECBUS_STATE_TDM_SYNC_DONE) && (ScalerGetBit(P2A_00_OCBUS_CTRL_00, _BIT6) == _BIT6))
                        {
                            ...
                            if(ScalerMHLRx2MscSendCommand(_MSC_SET_HPD, _MSC_NULL_ADDRESS, _MSC_NULL_VALUE) == _SUCCESS)
                            {
                                ...
                                ScalerMHLRx2MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_FEAT_REQ);

                                DebugMessageMHL("D2 MHL HPD Done", (0x10 | (GET_MHL3_RX2_ECBUS_START_UP_TMOUT())));
                    ...
                    {
                        if((GET_MHL3_RX2_WAIT_SRC_RAP_CBUS_UP_TMOUT() == _TRUE) || (GET_MHL3_RX2_WAIT_SRC_MHL_VERSION_TMOUT() == _TRUE))
                        {
#if(_MHL3_ECBUS_STARTUP_DEBUG_MESSAGE == _ON)
                            DebugMessageMHL("-- 3-2 -- Wait Source Rap Mpde Up TMOUT", ((GET_MHL3_RX2_WAIT_SRC_RAP_CBUS_UP_TMOUT() << 1) | (GET_MHL3_RX2_WAIT_SRC_MHL_VERSION_TMOUT())));
#endif
                            // MHL3 Wait RAP_CBUS_UP TMOUT -> Back to MHL2 //
                            // Enable Stuck Low IRQ and Clear Stuck Low Flags //
                            ScalerSetBit(P2A_A4_CBUS_CTRL_04, ~(_BIT1 | _BIT0), _BIT0);

                            ScalerTimerDelayXms(80);

                            if(ScalerMHLRx2MscSendCommand(_MSC_SET_HPD, _MSC_NULL_ADDRESS, _MSC_NULL_VALUE) == _SUCCESS)
                            {
                                DebugMessageMHL("D2 MHL HPD Done", (0x20 | (GET_MHL3_RX2_ECBUS_START_UP_TMOUT())));

                                SET_MHL_RX2_PROCESS_STATE(_MHL_STATE_HPD_DONE);
                            }
                        }
                        else if((GET_MHL3_RX2_ECBUS_START_UP_TMOUT() == _TRUE) && (GET_MHL3_RX2_ECBUS_MODE() == _ON))
                        {
#if(_MHL3_ECBUS_STARTUP_DEBUG_MESSAGE == _ON)
                            DebugMessageMHL("-- 3-3 -- GET_MHL3_ECBUS_START_UP_TMOUT", GET_MHL3_RX2_ECBUS_START_UP_TMOUT());
#endif
            ...
            case _MHL_STATE_HPD_DONE:
                ...
                if(GET_MHL3_RX2_ECBUS_MODE() == _ON)
                {
                    ...
                    if(GET_MHL3_RX2_MSC_FEAT_REQ_UNCOMPLETE() == _FALSE)
                    {
                        if(ScalerMHLRx2MscSendCommand(_MSC_WRITE_STAT, _MSC_LINK_MODE, _MSC_PATH_EN) == _SUCCESS)
                        {
                            DebugMessageMHL("D2 MHL Path Enabled", 0x10);
            ...
            case _MHL_STATE_PATH_EN_DONE:
                ...
                if(GET_TMDS_RX2_PHY_SET() == _TRUE)
                {
                    ...
                    if(GET_MHL3_RX2_ECBUS_MODE() == _ON)
                    {
                        ScalerMHLRx2MscSendCommand(_MSC_WRITE_XSTAT, _MSC_91_AVLINK_MODE_STATUS, _MSC_TMDS_NORMAL);
                    }
                ...
                {
                    if((GET_MHL3_RX2_ECBUS_MODE() == _ON) && (ScalerMHL3Rx2DataSignalDetect() == _TRUE))
                    {
                        if((GET_MHL3_RX2_AV_LINK_RATE() == _ECBUS_LINK_RATE_NULL) &&
                           (GET_MHL3_RX2_AV_LINK_N_CODE() == _ECBUS_N_CODE_NULL) &&
                           (ScalerGetByte(P2D_2C_HDCP_I2C_CTRL_6) == 0x10))
                        {
                            SET_MHL3_RX2_AV_LINK_RATE(_ECBUS_LINK_RATE_1_5);
                            SET_MHL3_RX2_AV_LINK_N_CODE(_ECBUS_N_CODE_1_5);
                            ...
                            DebugMessageMHL("D2 HDCP2.2 Auth Done", 0x00);
                        }
            ...
            case _MHL_STATE_BIST_TESTING:
                ...
                if((GET_MHL3_RX2_ECBUS_START_UP_TMOUT() == _TRUE) && (GET_MHL3_RX2_BIST_TESTING() == _ON))
                {
#if(_MHL3_BIST_FUNCTION_DEBUG_MESSAGE == _ON)
                    DebugMessageMHL("D2 BIST eCBUS Start Up Fail", 0x00);
#endif
                    ScalerMHLRx2CBUSResetProc();
                }

                if((GET_MHL3_RX2_ECBUS_PROCESS_STATE() == _MHL3_ECBUS_STATE_ECBUS_DONE) && (GET_MHL3_RX2_BIST_TESTING() == _ON))
                {
#if(_MHL3_BIST_FUNCTION_DEBUG_MESSAGE == _ON)
                    DebugMessageMHL("5 BIST Done", ScalerGetBit(P76_01_MHL_FIFO_CTRL, _BIT0));
#endif
                    ScalerMHLRx2CBUSResetProc();
                }
...
bit ScalerMHLRx2MscRAPHandler(void)
{
    ...
    if((bit)ScalerGetBit(P2A_D4_CBUS_CTRL_34, _BIT3) == _TRUE)
    {
        ...
        if((ScalerGetByte(P2A_D2_CBUS_CTRL_32) != 0x00) &&
           ((GET_MHL_RX2_ADOPTER_ID_HBYTE() == _UTS800_SL863_ADOPTER_ID_HBYTE) &&
            (GET_MHL_RX2_ADOPTER_ID_LBYTE() == _UTS800_SL863_ADOPTER_ID_LBYTE)))
        {
            // Clear Flag
        ...
        switch(ScalerGetByte(P2A_DC_CBUS_CTRL_3C))
        {
            ...
            case _MSC_RAP_CBUS_MODE_DOWN:
                ...
                if(ScalerMHLRx2MscSendCommand(_MSC_MSG, _MSC_RAPK, _MSC_RAPK_NO_ERROR) == _TRUE)
                {
                    ScalerMHLRx2CBUSResetProc();
            ...
            case _MSC_RAP_CBUS_MODE_UP:
                ...
                if(ScalerMHLRx2MscSendCommand(_MSC_MSG, _MSC_RAPK, _MSC_RAPK_NO_ERROR) == _TRUE)
                {
                    if(GET_MHL3_RX2_ECBUS_MODE() == _OFF)
                    {
                        //-------- Disable Stuck Low IRQ and Clear Stuck Low Flags --------//
                        ...
                        ScalerSetBit(P2B_3C_ECBUS_SRAM_BIST, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);
