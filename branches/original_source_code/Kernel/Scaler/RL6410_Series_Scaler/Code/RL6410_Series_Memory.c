...
void ScalerMemoryFrameSyncSetDt(void)
{
       ...
       (ulIVStoDVSPixels >= (((g_stMDomainInputData.usHTotal >> 1) + 64) >> ucEOshift)))
    {
        ...
        PDATA_WORD(4) = (ulIVStoDVSPixels - (g_stMDomainInputData.usHTotal >> (1 + ucEOshift)) - ((DWORD)g_stMDomainInputData.usHTotal * PDATA_WORD(2))) >> (4 + ucFBshift);
        pData[10] = (ulIVStoDVSPixels - ((DWORD)g_stMDomainInputData.usHTotal * PDATA_WORD(2)) - (PDATA_WORD(3) << (4 + ucFBshift))) >> ucFBshift;
        pData[11] = (ulIVStoDVSPixels - ((DWORD)g_stMDomainInputData.usHTotal * PDATA_WORD(2)) - (PDATA_WORD(4) << (4 + ucFBshift))) >> ucFBshift;
    }
    else
    {
