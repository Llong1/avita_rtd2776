...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_MHLRx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//****************************************************************************
void ScalerMHLRx0ModeMHLSetting(void);
void ScalerMHLRx0ModeHdmiSetting(void);
void ScalerMHLRx0InitialSettings(void);
void ScalerMHLRx0CBUSPhySwitch(bit bSelect);
void ScalerMHLRx0CBUSSwitchClk(EnumClkSel enumClockSel);
void ScalerMHLRx0CBUSResetProc(void);
void ScalerMHLRx0PowerOffProc(void);
void ScalerMHLRx0IntHandler_EXINT0(void);
bit ScalerMHLRx0MscReadDevCapabilities(EnumMHLMscOffset enumMSCOffset, BYTE *pucData);
BYTE ScalerMHLRx0MscFIFOSendCommand(EnumMHLMscCommand enumMSCCommand, EnumMHLMscOffset enumMSCOffset, EnumMHLMscData enumMSCValue, BYTE ucDataLength, BYTE *pucData);
void ScalerMHLRx0MscCheckDeviceINT(void);
bit ScalerMHLRx0MscHandler(EnumDisplayRegion enumMappedRegion, BYTE ucModeState);
bit ScalerMHLRx0MscRAPHandler(void);

#if(_MHL2_UCP_SUPPORT == _ON)
BYTE ScalerMHLRx0MscUCPGetCommand(BYTE *pucKeyCode);
#endif

bit ScalerMHLRx0PSPreDetect(void);
bit ScalerMHLRx0NormalPreDetect(void);
...
void ScalerMHLRx0ModeMHLSetting(void)
{
    ...
    CLR_MHL_RX0_SWITCH_PORT_DISCOVERY_DONE();
    ...
    // Enable MHL Mode
    ScalerSetBit(PB_A0_CMU_00, ~(_BIT7 | _BIT6), (_BIT7 | _BIT6));
...
#elif(_D0_DDC_CHANNEL_SEL == _DDC4)
    // Disable DDC Channel
...
#elif(_D0_DDC_CHANNEL_SEL == _DDC5)
    // Disable DDC Channel
...
void ScalerMHLRx0InitialSettings(void)
{
    ...
    ScalerSetBit(P28_A4_CBUS_CTRL_04, ~_BIT7, _BIT7);

    // Set MHL Wake pulse boundary 14~26
    ScalerSetBit(P28_A4_CBUS_CTRL_04, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT4 | _BIT3));
...
void ScalerMHLRx0CBUSSwitchClk(EnumClkSel enumClockSel)
{
    if((enumClockSel == _M2PLL_CLK) && (ScalerGetBit(P28_A0_CBUS_CTRL_00, _BIT4) != 0x00))
    {
        ...
        ScalerSetBit(P28_A0_CBUS_CTRL_00, ~(_BIT4 | _BIT3 | _BIT2), 0x00);

        // Clear Wake Up Pulses Flag
        ScalerSetBit(P28_A0_CBUS_CTRL_00, ~_BIT3, _BIT3);

        // Sys Clk Divider
        ScalerSetBit(P28_A1_CBUS_CTRL_01, ~(_BIT7 | _BIT6 | _BIT5), (_BIT6 | _BIT5));

        // Set Debounce For Core Function Set to 2 cycles
        ScalerSetBit(P28_A2_CBUS_CTRL_02, ~(_BIT7 | _BIT6 | _BIT5), _BIT6);

        // Upper Bound of SYNC Pulse Low Time Period
        ScalerSetByte(P28_AF_CBUS_CTRL_0F, 0xE1);

        // Lower Bound of SYNC Pulse Low Time Period
        ScalerSetByte(P28_B0_CBUS_CTRL_10, 0x83);

        // Upper Bound of SYNC Pulse High Time Period
        ScalerSetByte(P28_B1_CBUS_CTRL_11, 0x55);

        // Lower Bound of SYNC Pulse High Time Period
        ScalerSetByte(P28_B2_CBUS_CTRL_12, 0x25);

        // Absolute Threshold Time
        ScalerSetByte(P28_B4_CBUS_CTRL_14, 0x5A);

        // Parity Bit Time
        ScalerSetByte(P28_B5_CBUS_CTRL_15, 0x75);

        // Parity Error Limit
        ScalerSetBit(P28_B6_CBUS_CTRL_16, ~(_BIT7 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x0E);

        // Ack Bit Initial Falling Edge
        ScalerSetBit(P28_B7_CBUS_CTRL_17, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x04);

        // Ack Bit Drive Low Time
        ScalerSetBit(P28_B8_CBUS_CTRL_18, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x3A);

        // Requester Bit Time
        ScalerSetByte(P28_BA_CBUS_CTRL_1A, 0x75);

        // Check Received Ack Bit's Falling Edge
        ScalerSetBit(P28_BC_CBUS_CTRL_1C, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x7A);

        // Check Received Ack Bit's Driving Low Period Upper Bound
        ScalerSetBit(P28_BD_CBUS_CTRL_1D, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x4C);

        // Check Received Ack Bit's Driving Low Period Lower Bound
        ScalerSetBit(P28_BE_CBUS_CTRL_1E, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x24);

        // Actively Driving High Time for CBUS
        ScalerSetBit(P28_C0_CBUS_CTRL_20, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x18);

        // CBUS Requester Transmit Opportunity after Arbitration
        ScalerSetByte(P28_C2_CBUS_CTRL_22, 0x75);

        // CBUS Requester Continue After Ack
        ScalerSetByte(P28_C3_CBUS_CTRL_23, 0x3A);

        // Link Layer Timeout
        ScalerSetBit(P28_C5_CBUS_CTRL_25, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), (_BIT4 | _BIT3 | _BIT2));

        // Set IIC SCL Clock Frequency
        ScalerSetByte(P28_F9_CBUS_CTRL_59, 0x11);
    ...
    else if((enumClockSel == _IOSC_CLK) && (ScalerGetBit(P28_A0_CBUS_CTRL_00, _BIT4) != _BIT4))
    {
        // Gate CBUS System Clk First (In Case of Digital Clk Glitch)
        ScalerSetBit(P28_A0_CBUS_CTRL_00, ~_BIT5, 0x00);

        // MHL System Clk select to IOSC (Bypass GDI Clk)
        ScalerSetBit(P28_A0_CBUS_CTRL_00, ~(_BIT4 | _BIT3 | _BIT2), _BIT4);
...
void ScalerMHLRx0CBUSResetProc(void)
{
    ...
    SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_INITIAL);
...
// Output Value : None
//--------------------------------------------------
void ScalerMHLRx0IntHandler_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(P28_A0_CBUS_CTRL_00, (_BIT2 | _BIT0)) == (_BIT2 | _BIT0))
    {
        ...
        if(ScalerGetBit_EXINT(PB_A2_CMU_02, _BIT4) == 0x00)
        {
        ...
        // Enable RX_EN
        ScalerSetBit_EXINT(PB_A1_CMU_01, ~_BIT7, _BIT7);

        // Port 0 Z0 Enable
        ScalerSetBit_EXINT(PB_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT6 | _BIT5 | _BIT4));

        // Clear HDCP Flags
        ScalerSetBit_EXINT(P2_A9_TMDS_ABC0, ~(_BIT7 | _BIT6 | _BIT5), (_BIT7 | _BIT6 | _BIT5));

        SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_DISCOVERY_DONE);
    ...
    if(ScalerGetBit_EXINT(P28_A4_CBUS_CTRL_04, (_BIT1 | _BIT0)) == (_BIT1 | _BIT0))
    {
        ...
        // Port 0 Z0 Disable
        ScalerSetBit_EXINT(PB_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), 0x00);

        // Disable Auto K-offset Auto Mode
        ScalerSetBit_EXINT(PB_AD_CDR_04, ~_BIT2, 0x00);
...
bit ScalerMHLRx0MscReadDevCapabilities(EnumMHLMscOffset enumMSCOffset, BYTE *pucData)
{
    ...
    {
        ucResultPacket = ScalerMHLRx0MscFIFOSendCommand(_MSC_READ_DEVCAP, enumMSCOffset, _MSC_NULL_VALUE, 0, pucData);
...
BYTE ScalerMHLRx0MscFIFOSendCommand(EnumMHLMscCommand enumMSCCommand, EnumMHLMscOffset enumMSCOffset, EnumMHLMscData enumMSCValue, BYTE ucDataLength, BYTE *pucData)
{
    ...
    while(--ucTimeOut != 0)
    {
        ...
        else if((ScalerGetBit(P28_F7_CBUS_CTRL_57, _BIT7) == _BIT7) && (ScalerGetByte(P28_D2_CBUS_CTRL_32) == _MSC_PROTOCOL_ERROR))
        {
            ...
            ScalerSetByte(P28_D2_CBUS_CTRL_32, 0xFF);
...
void ScalerMHLRx0MscCheckDeviceINT(void)
{
    ...
    if((bit)ScalerGetBit(P28_CE_CBUS_CTRL_2E, _BIT7) == _TRUE)
    {
            ...
            ((bit)ScalerGetDataPortBit(P28_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_DCAP_CHG) == _TRUE)))
        {
            if(GET_MHL_RX0_MHL_VERSION() == _MHL_VERSION_NULL)
            {
                ...
                if(ScalerMHLRx0MscReadDevCapabilities(_MSC_MHL_VERSION_01, &g_ucMHLRx0MhlVersion) == _TRUE)
                {
            ...
            if((GET_MHL_RX0_ADOPTER_ID_DONE() == _FALSE) && (GET_MHL_RX0_READY_TO_TRANSMIT() == _TRUE))
            {
                if((ScalerMHLRx0MscReadDevCapabilities(_MSC_ADOPTER_ID_H_03, &g_ucMHLRx0AdopterIDH) == _TRUE) &&
                   (ScalerMHLRx0MscReadDevCapabilities(_MSC_ADOPTER_ID_L_04, &g_ucMHLRx0AdopterIDL) == _TRUE))
                {
                    DebugMessageRx0("D0 MHL Adopter IDH", g_ucMHLRx0AdopterIDH);
                    DebugMessageRx0("D0 MHL Adopter IDL", g_ucMHLRx0AdopterIDL);
        ...
        //------------------------------------//
        if((bit)ScalerGetDataPortBit(P28_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_REQ_WRT) == _TRUE)
        {
            if(ScalerMHLRx0MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_GRT_WRT) == _SUCCESS)
            {
        ...
        if((bit)ScalerGetDataPortBit(P28_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_3D_REQ) == _TRUE)
        {
            ...
            if(ScalerMHLRx0MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_REQ_WRT) == _SUCCESS)
            {
                usTmp = 400;
                ...
                if(ScalerMHLRx0MscSendWriteBurst(0x40, ucBirstLength, &pucWriteBurstTmp[0], _MHL_WRITE_BURST_WITH_REQ) == _SUCCESS)
                {
                    if(ScalerMHLRx0MscSendCommand(_MSC_SET_INT, _MSC_RCHANGE_INT, _MSC_REQ_WRT) == _SUCCESS)
                    {
                        usTmp = 400;
                        ...
                        if(ScalerMHLRx0MscSendWriteBurst(0x40, ucBirstLength, &pucWriteBurstTmp[ucBirstLength], _MHL_WRITE_BURST_WITH_REQ) == _SUCCESS)
                        {
        ...
        // Clear Flag
        ScalerSetByte(P28_CE_CBUS_CTRL_2E, (BYTE)_BIT7);
    }
    ...
    if((bit)ScalerGetBit(P28_CE_CBUS_CTRL_2E, _BIT6) == _TRUE)
    {
        // Read Source Adopter ID
        if((GET_MHL_RX0_ADOPTER_ID_DONE() == _FALSE) && (GET_MHL_RX0_READY_TO_TRANSMIT() == _TRUE))
        {
               ...
               (ScalerMHLRx0MscReadDevCapabilities(_MSC_ADOPTER_ID_L_04, &g_ucMHLRx0AdopterIDL) == _TRUE))
            {
                ...
                ScalerSetBit(P28_CE_CBUS_CTRL_2E, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT6);
            }
        }
    }
    ...
    if(((GET_MHL_RX0_ADOPTER_ID_HBYTE() == _HTC_BUTTERFLY_ADOPTER_ID_HBYTE) && (GET_MHL_RX0_ADOPTER_ID_LBYTE() == _HTC_BUTTERFLY_ADOPTER_ID_LBYTE)) &&
       (GET_MHL_RX0_SENT_MSC_DCAP_RDY() == _FALSE) && (GET_MHL_RX0_PROCESS_STATE() == _MHL_STATE_SET_DEV_CAP_DONE))
    {
        ScalerMHLRx0MscSendCommand(_MSC_WRITE_STAT, _MSC_CONNECTED_RDY, _MSC_DCAP_RDY);
    ...
    {
        ScalerSetByte(P28_C8_CBUS_CTRL_28, _BIT6);

        ScalerSetByte(P28_CE_CBUS_CTRL_2E, _BIT5);
    }

    if(ScalerGetBit(P28_CE_CBUS_CTRL_2E, _BIT4) == _BIT4)
    {
        ScalerSetByte(P28_C8_CBUS_CTRL_28, 0x00);

        ScalerSetByte(P28_CE_CBUS_CTRL_2E, _BIT4);
    }
...
bit ScalerMHLRx0MscHandler(EnumDisplayRegion enumMappedRegion, BYTE ucModeState)
{
    ...
    switch(GET_MHL_RX0_PROCESS_STATE())
    {
        ...
        case _MHL_STATE_DISCOVERY_DONE:
            ...
            if(ScalerMHLRx0MscSendCommand(_MSC_WRITE_STAT, _MSC_CONNECTED_RDY, _MSC_DCAP_RDY) == _SUCCESS)
            {
                ScalerTimerActiveTimerEvent(SEC(1), _SCALER_TIMER_EVENT_MHL_RX0_SWITCH_PORT_DISCOVERY_DONE);

                DebugMessageDigital("D0 MHL Dev Cap RDY", 0x01);
                SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_DEV_CAP_RDY);
            }
    ...
    if((enumMappedRegion != _DISPLAY_RGN_NONE) && (ucModeState != _MODE_STATUS_INITIAL) && (ucModeState != _MODE_STATUS_POWER_OFF))
    {
        switch(GET_MHL_RX0_PROCESS_STATE())
        {
            case _MHL_STATE_DEV_CAP_RDY:
                ...
                if(ScalerMHLRx0MscSendCommand(_MSC_SET_HPD, _MSC_NULL_ADDRESS, _MSC_NULL_VALUE) == _SUCCESS)
                {
                    DebugMessageDigital("D0 MHL HPD Done", 0x01);
                    SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_HPD_DONE);
                }
            ...
            case _MHL_STATE_HPD_DONE:
                ...
                if((GET_MHL_RX0_ADOPTER_ID_HBYTE() == _ZTE_NUBIA_Z5_ADOPTER_ID_HBYTE) && (GET_MHL_RX0_ADOPTER_ID_LBYTE() == _ZTE_NUBIA_Z5_ADOPTER_ID_LBYTE))
                {
                    ScalerTimerDelayXms(50);
                }

                if(ScalerMHLRx0MscSendCommand(_MSC_WRITE_STAT, _MSC_LINK_MODE, _MSC_PATH_EN) == _SUCCESS)
                {
                    DebugMessageDigital("D0 MHL Path Enabled", 0x01);
                    SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_PATH_EN_DONE);
                    SET_MHL_RX0_READY_TO_RESEND();
            ...
            case _MHL_STATE_PATH_EN_DONE:
                ...
                {
                    SET_MHL_RX0_PROCESS_STATE(_MHL_STATE_DONE);
                }
...
bit ScalerMHLRx0MscRAPHandler(void)
{
    ...
    {
        if((ScalerGetByte(P28_DC_CBUS_CTRL_3C) == _MSC_RAP_POLL) ||
           (ScalerGetByte(P28_DC_CBUS_CTRL_3C) == _MSC_RAP_CONTENT_ON) ||
           (ScalerGetByte(P28_DC_CBUS_CTRL_3C) == _MSC_RAP_CONTENT_OFF))
        {
            // Reply RAPK and No Error
            ScalerMHLRx0MscSendCommand(_MSC_MSG, _MSC_RAPK, _MSC_RAPK_NO_ERROR);
        }
        else
        {
            // Reply RAPK and Ineffective Code
            ScalerMHLRx0MscSendCommand(_MSC_MSG, _MSC_RAPK, _MSC_RAPK_UNRECOGNIZE_CODE);
        }
...
{
    // Check UCP Command
    if(((bit)ScalerGetBit(P28_D4_CBUS_CTRL_34, _BIT1) == _TRUE) && (ScalerGetByte(P28_D6_CBUS_CTRL_36) == _MSC_UCP))
    {
        // Clear Flag
        ScalerSetByte(P28_D4_CBUS_CTRL_34, (BYTE)_BIT1);

        pucKeyCode[0] = ScalerGetByte(P28_D7_CBUS_CTRL_37);
...
bit ScalerMHLRx0PSPreDetect(void)
{
    ...
    if(ScalerGetBit(P28_A0_CBUS_CTRL_00, (_BIT5 | _BIT4 | _BIT3)) == (_BIT5 | _BIT4 | _BIT3))
    {
        return _TRUE;
    }

    // Pre-Detect True By Switch Back to MHL Port
    if(GET_MHL_RX0_SWITCH_PORT_DISCOVERY_DONE() == _TRUE)
    {
        CLR_MHL_RX0_SWITCH_PORT_DISCOVERY_DONE();
        return _TRUE;
    }
    ...
    // Disable clk lane Z0
    ScalerSetBit(PB_A2_CMU_02, ~_BIT7, 0x00);
    ...
    if((ScalerGetBit(P2_E6_HDMI_FREQDET_CTRL, (_BIT6 | _BIT2)) == _BIT6) && (ScalerGetBit(P2_EF_HDMI_ERROR_TH, _BIT6) == _BIT6))
    {
        ...
        // Enable clk lane Z0
        ScalerSetBit(PB_A2_CMU_02, ~_BIT7, _BIT7);
